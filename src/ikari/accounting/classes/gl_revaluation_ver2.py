...
class C_GL_Revaluation_ver2:
            ...
            if result[0]:
                ...
                if result[0]:
                    ...
                    if result[0]:
                        ...
                        if result[0]:
                            result = self.createReversalEntries()
        return result
    ...
    def createReversalEntries(self):
        result = False
        status = None
        success = Fail = 0
        if len(self.EntrySets):
            for EntrySet in self.EntrySets:
                if float(EntrySet['amount']) != 0.000000:
                    try:
                        ...
                        gl_rv_journal.reverse_to_period = 1
                        rev_yy = int(EntrySet['fiscal_year'])
                        rev_mm = int(EntrySet['period_to']) + 1
                        bulan = rev_mm + 1
                        year_revers = rev_yy
                        if rev_mm == 12:
                            bulan = 1
                            year_revers = rev_yy + 1
                            rev_yy = int(EntrySet['fiscal_year']) + 1
                            rev_mm = 1
                        gl_rv_journal.reverse_to_period_val = str(year_revers) + '-' + str(bulan) + '-01'
                        gl_rv_journal.perd_month = rev_mm
                        gl_rv_journal.perd_year = rev_yy
                        gl_rv_journal.is_auto_reversed_entry = True
                        gl_rv_journal.save()
                        if not self.createReversalTransactions(EntrySet, gl_rv_journal.id):
                            self.FailedEntries.append(EntrySet)
                            Fail += 1
                        success += 1
                    except Exception as e:
    ...
    def createReversalTransactions(self, EntrySet, journal_id):
        result = True
        try:
            revaluation_code = RevaluationCode.objects.get(pk=EntrySet['revaluation_code_id'])
            ...
            MainAccount.account_id = EntrySet['acc_id']
            MainAccount.currency_id = EntrySet['currency_id']
            MainAccount.is_credit_account = (False, True)[EntrySet['amount'] >= 0]
            MainAccount.is_debit_account = (True, False)[EntrySet['amount'] >= 0]
            MainAccount.amount = 0
            MainAccount.reference = 'REVERSING ENTRY'
            MainAccount.functional_currency_id = self.d_company.currency_id
            MainAccount.total_amount = 0
            MainAccount.functional_amount = math.fabs(Decimal(EntrySet['amount']))
            MainAccount.exchange_rate = Decimal(EntrySet['rate'])
            ...
            MainAccount.update_by = self.request.user.id
            MainAccount.company_id = self.company_id
            MainAccount.functional_balance_type = (BALANCE_TYPE_DICT['Debit'],
                                                   BALANCE_TYPE_DICT['Credit'])[MainAccount.is_debit_account]
            MainAccount.save()
            ...
            ReverseAccount.is_credit_account = not MainAccount.is_credit_account
            ReverseAccount.amount = 0
            ReverseAccount.reference = 'REVERSING ENTRY'
            ReverseAccount.functional_currency_id = self.d_company.currency_id
            ReverseAccount.total_amount = 0
            ReverseAccount.functional_amount = math.fabs(Decimal(EntrySet['amount']))
            ReverseAccount.exchange_rate = Decimal(EntrySet['rate'])
            ...
            ReverseAccount.update_by = self.request.user.id
            ReverseAccount.company_id = self.company_id
            ReverseAccount.functional_balance_type = (BALANCE_TYPE_DICT['Debit'],
                                                      BALANCE_TYPE_DICT['Credit'])[ReverseAccount.is_debit_account]
            ReverseAccount.save()
        except:
            result = False
        return result
