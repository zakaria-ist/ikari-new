...
class Print_Tax_Invoice:
    ...
    def _header_footer(canvas, doc, order_id, print_header, company_id):
        ...
        # ===========================Header Title ===========================
        header_data = []
        row1_info1 = Paragraph("TAX INVOICE", styles['CustomTitle'])
        header_data.append([row1_info1])
        ...
        if order:
            ...
            if order.customer:
                ...
                row3_info1 = Paragraph(order.customer.name if order.customer.name else '', styles['LeftAlign'])
    ...
    def print_report(self, order_id, print_header, company_id, part_group):
        ...
        if company and order and order_item_list:
            ...
            for index, my_item in enumerate(order_item_list):
                ...
                item_list_data.append([intcomma("%.0f" % my_item.line_number),  my_item.customer_po_no, my_item.item.code,
                                       intcomma("%.2f" % round_number(my_item.quantity)), '', intcomma("%.5f" % round_number(my_item.price, 5)),
                ...
                if int(part_group):
                    item_list_data.append(['', my_item.refer_number if my_item.refer_number else '', my_item.item.name[:34] if my_item.item.name else '',
            ...
            # ===========================Footer Data============================
            footer_data = []
            footer_data.append(['', '', 'TOTAL:', intcomma("%.2f" % round_number(total_quantity)), '', '', ''])
            ...
            if int(order.currency_id) == int(company.currency_id):
                ...
                try:
                    exchange_rate = currencies.models.ExchangeRate.objects.filter(company_id=company_id, is_hidden=0, from_currency_id=order.currency_id,
                                                                                  to_currency_id=company.currency_id,
                                                                                  exchange_date__month=order.document_date.month,
                                                                                  exchange_date__year=order.document_date.year, flag='ACCOUNTING').last()
                    if exchange_rate:
            ...
            remark_data.append(['EXCHANGE RATE', ':', intcomma("%.4f" % round_number(float(exchange_rate), 4)), '',
                                'TOTAL B/F TAX ', ':', intcomma(decimal_place % round_number(float(order.subtotal)))])
            ...
            remark_data = []
            remark_data.append([Paragraph('REMARKS:', styles['BoldText']), Paragraph('for', styles['ItalicText']),
