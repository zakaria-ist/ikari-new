...
class GLFunction_Balance_Batch_XLS:
    ...
    def print_report(self, company_id, issue_from, issue_to, status_type, acc_list, sp_period):
        ...
        worksheet.write(20, 3, 'Description/', center_bold)
        ...
        worksheet.write(21, 3, 'Reference', center_line)
        worksheet.write(21, 4, 'Posting Seq.', center_line)
        worksheet.write(21, 5, 'Batch-Entry', center_line)
        ...
        if sp_period == 'CLS':
            transaction_item_list = Transaction.objects.select_related('journal__batch').select_related('journal').select_related('account').filter(
                ...
                .exclude(journal__batch__status__in=(int(STATUS_TYPE_DICT['Deleted']), int(STATUS_TYPE_DICT['Open']),
                                                    int(STATUS_TYPE_DICT['ERROR']), int(STATUS_TYPE_DICT['Prov. Posted']))
        ...
        if account_item_list:
            for i, mAccount in enumerate(account_item_list):
                ...
                for month in range(-1, number_of_month):
                    ...
                    transaction_item_list_account = transaction_item_list.filter(account_id=mAccount.id,
                        ...
                        # .order_by('source_type', 'journal__batch__posting_sequence')
                    ...
                    if (len(transaction_item_list_account) or total_balance) and print_account_checker:
                        ...
                        worksheet.write(printing_row, printing_col + 9, round_number(
                            total_balance), dec_format if is_decimal else num_format)
                        printing_row += 1
                    ...
                    if transaction_item_list_account:
                        for j, mItem in enumerate(transaction_item_list_account):
                            ...
                            worksheet.write(printing_row, printing_col + 4, posting_sequence)
                            worksheet.write(printing_row, printing_col + 5, batch_no)
                            worksheet.write(printing_row, printing_col + 6, round_number(mItem.functional_amount)
                                            if mItem.functional_balance_type == BALANCE_TYPE_DICT['Debit'] else None, dec_format if is_decimal else num_format)
                            worksheet.write(printing_row, printing_col + 7, round_number(mItem.functional_amount)
                                            if mItem.functional_balance_type == BALANCE_TYPE_DICT['Credit'] else None, dec_format if is_decimal else num_format)
                            printing_row += 1
                            worksheet.write(printing_row, printing_col + 3, transaction_detail_list[0])
                            printing_row += 1
                        ...
                        if (trx_printed and j == transaction_item_list_account.__len__() - 1):
                            ...
                            worksheet.write(printing_row, printing_col + 7, 'Net Change and Ending Balance for Fiscal Period ' +
                                            str(current_month) + ': ', right_bold)
                            worksheet.write(printing_row, printing_col + 8, round_number(
                                sum_amount_debit - sum_amount_credit), border_top_bot_dec if is_decimal else border_top_bot_num)
                            worksheet.write(printing_row, printing_col + 9, round_number(
                                total_balance), border_top_bot_dec if is_decimal else border_top_bot_num)
                            printing_row += 1
                        ...
                        if(not trx_printed) and printing_row > 0:
                            printing_row -= 1
                            if total_balance == 0 and printing_row > 0:
                                printing_row -= 1
                                account_counter -= 1

                # Account Total
                if sum_amount_credit_acc != 0 or sum_amount_debit_acc != 0:
                    worksheet.write(printing_row, printing_col + 5, 'Total: ' + str(mAccount.name) + ' ' + str(curr_year), right_bold)
                    worksheet.write(printing_row, printing_col + 6, round_number(
                        sum_amount_debit_acc), border_top_bot_dec if is_decimal else border_top_bot_num)
                    worksheet.write(printing_row, printing_col + 7, round_number(
                        sum_amount_credit_acc), border_top_bot_dec if is_decimal else border_top_bot_num)
                    worksheet.write(printing_row, printing_col + 8, round_number(sum_amount_debit_acc -
                                    sum_amount_credit_acc), border_top_bot_dec if is_decimal else border_top_bot_num)
                    worksheet.write(printing_row, printing_col + 9, round_number(
                        total_balance_acc), border_top_bot_dec if is_decimal else border_top_bot_num)
                    printing_row += 1
            ...
            # GRAND TOTAL
            printing_row += 1
            worksheet.write(printing_row, printing_col + 5, 'Report Total: ', right_bold)
            worksheet.write(printing_row, printing_col + 6, round_number(g_debit),
                            border_top_bot_dec if is_decimal else border_top_bot_num)
            worksheet.write(printing_row, printing_col + 7, round_number(g_credit),
                            border_top_bot_dec if is_decimal else border_top_bot_num)
            worksheet.write(printing_row, printing_col + 8, round_number(g_net),
                            border_top_bot_dec if is_decimal else border_top_bot_num)
            worksheet.write(printing_row, printing_col + 9, round_number(grandtotal),
                            border_top_bot_dec if is_decimal else border_top_bot_num)
            printing_row += 2
