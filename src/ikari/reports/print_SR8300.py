...
class Print_SR8300:
    ...
    def _header_footer(canvas, doc, company_id, delivery_from, delivery_to, supp_list):
        ...
        # 1st row
        header_data = []
        row1_info1 = "SL8300 Sales / Purchase System"
        row1_info2 = "P/O and Goods Receive Matching Report By Supplier"
    ...
    def print_report(self, company_id, delivery_from, delivery_to, supplier_no, document_no):
        pdfmetrics.registerFont(TTFont('Arial', os.path.join(s.BASE_DIR, "static/fonts/arial.ttf")))
        pdfmetrics.registerFont(TTFont('Arial-Bold', os.path.join(s.BASE_DIR, "static/fonts/arial-bold.ttf")))
        ...
        order_item_list = OrderItem.objects.filter(is_hidden=0, order__is_hidden=0, order__company_id=company_id,
            ...
            .order_by('supplier__code', 'order__document_number')
        ...
        for i, mItem in enumerate(order_item_list):
            if supplier_code != mItem.supplier.code:
                gr_exists = False
                supplier_code = mItem.supplier.code
                table_data = []
            ...
            if supplier_code == mItem.supplier.code:
                ...
                if gr_refer_item:
                    ...
                    for refer_item in gr_refer_item:
                        table_data = []
                        table_data.append(['G/R', refer_item.order.document_number,
                                           refer_item.line_number, refer_item.order.document_date.strftime("%d/%m/%Y"),
                                           refer_item.item.code, '', intcomma("%.2f" % refer_item.quantity)])
