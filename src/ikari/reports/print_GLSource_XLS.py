...
class GLSource_XLS:
    ...
    def print_report(self, company_id, issue_from, issue_to, status_type, acc_list, sp_period):
        ...
        worksheet.merge_range('G20:H20', '--------------- Source Currency --------------------', center_bold)
        worksheet.merge_range('I20:J20', '--------------- Functional Currency ----------------', center_bold)
        ...
        worksheet.write(21, 5, 'Curr', center_line)
        worksheet.write(21, 6, 'Debits', right_bold)
        worksheet.write(21, 7, 'Credits', right_bold)
        worksheet.write(21, 8, 'Debits', right_bold)
        worksheet.write(21, 9, 'Credits', right_bold)
        ...
        if account_item_list:
            for i, mAccount in enumerate(account_item_list):
                ...
                for month in range(-1, number_of_month):
                    ...
                    worksheet.write(printing_row, printing_col + 8,
                                    float(db_balance.replace(',', '')), dec_format if is_decimal_f else num_format)
                    worksheet.write(printing_row, printing_col + 9,
                                    float(cr_balance.replace(',', '')), dec_format if is_decimal_f else num_format)
                    printing_row += 1
                    ...
                    if transaction_item_list_account:
                        for j, mItem in enumerate(transaction_item_list_account):
                            ...
                            worksheet.write(printing_row, printing_col + 7, 
                ...
                worksheet.write(printing_row, printing_col + 8, db_balance,
                                dec_format if is_decimal_f else num_format)
                worksheet.write(printing_row, printing_col + 9, cr_balance,
                                dec_format if is_decimal_f else num_format)
                printing_row += 1
        ...
        worksheet.write(printing_row, printing_col + 7, 'Report Total:', right_bold)
        worksheet.write(printing_row, printing_col + 8, round_number(accumulate_debit_amount),
                        border_top_bot_dec if is_decimal_f else border_top_bot_num)
        worksheet.write(printing_row, printing_col + 9, round_number(accumulate_credit_amount),
                        border_top_bot_dec if is_decimal_f else border_top_bot_num)
        printing_row += 2
