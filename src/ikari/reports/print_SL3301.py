...
class Print_SL3301:
    ...
    @staticmethod
    def _header_footer(canvas, doc, company_id, delivery_from, delivery_to, cpo_list):
        ...
        # 1st row
        header_data = []
        row1_info1 = "SL3301 Sales / Purchase System"
        row1_info2 = "Sales Order Check List"
        header_data.append([row1_info1, row1_info2])
        # 2nd row
        company = Company.objects.get(pk=company_id)
        row2_info1 = company.name
        row2_info2 = "Grouped by Customer PO No., Transaction Code, Document No."
        ...
        # 4th row
        if len(cpo_list):
            row4_info1 = "Customer PO No. : [" + cpo_list[0] + " ] - [ " + cpo_list[-1] + " ]"
        else:
        ...
        table_header.append(['P/O No.', 'P/O Ln.   Due Date', 'Sup. Code', 'P/O Gen.', '', '', ''])
    ...
    def print_report(self, company_id, delivery_from, delivery_to, document_no, customer_no, customer_po):
        pdfmetrics.registerFont(TTFont('Arial', os.path.join(s.BASE_DIR, "static/fonts/arial.ttf")))
        pdfmetrics.registerFont(TTFont('Arial-Bold', os.path.join(s.BASE_DIR, "static/fonts/arial-bold.ttf")))
        ...
        order_item_list = OrderItem.objects.filter(is_hidden=0, order__is_hidden=0, order__company_id=company_id,
            ...
            .order_by('customer_po_no', 'order__document_number')
        ...
        if len(cpo_list):
            order_item_list = order_item_list.filter(id__in=cpo_list)
            cpo_list = [order_item_list.filter(id=cpo_list[0]).first().customer_po_no,
                        order_item_list.filter(id=cpo_list[-1]).first().customer_po_no]
        ...
        for i, mItem in enumerate(order_item_list):
            if customer_po_no != mItem.customer_po_no:
                ...
                if total_qty > 0:
                    table_data = []
                    table_data.append(['', '', '', 'Cust. PO Total : ', intcomma("%.2f" % total_qty), '', intcomma("%.2f" % total_amount)])
                    table_data.append(['', '', '', '', '', '', ''])
                ...
                table_data = []
                table_data.append([
                    mItem.customer_po_no, '', '', '', '', '', ''])
                table_data.append([
                    mItem.order.document_number, mItem.order.customer.code + '   ' + mItem.order.customer.name[:33], '',
                    mItem.order.document_date.strftime("%d/%m/%Y"), mItem.order.customer.payment_term + ' Days',
                    mItem.order.customer.currency.code, intcomma("%.8f" % mItem.order.exchange_rate)])
                item_table = Table(table_data, colWidths=colWidths, rowHeights=rowHeights)
                item_table.setStyle(TableStyle(
                    [('INNERGRID', (0, 0), (-1, -1), 0.25, colors.transparent),
                     ('BOX', (0, 0), (-1, -1), 0.25, colors.transparent),
            ...
            if customer_po_no == mItem.customer_po_no:
                ...
                table_data = []
                table_data.append([
                    mItem.item.code, '', mItem.item.inv_measure.code if mItem.item.inv_measure else '' + '   ' + 
                    ...
                    intcomma("%.2f" % mItem.quantity), intcomma("%.5f" % mItem.price), intcomma("%.2f" % mItem.amount)])

                table_data.append([
                    po_refer_item.order.document_number if po_refer_item else '',
                    str(po_refer_item.line_number) + '   ' + po_refer_item.wanted_date.strftime("%d/%m/%Y") if po_refer_item else '',
                    po_refer_item.supplier.code if po_refer_item else '', 'Y' if po_refer_item else 'N', '', '', ''])
        ...
        doc.build(elements,
                  onFirstPage=partial(self._header_footer, company_id=company_id, delivery_from=delivery_from_obj.strftime('%d-%m-%Y'),
                                      delivery_to=delivery_to_obj.strftime('%d-%m-%Y'), cpo_list=cpo_list),
