...
from reports.print_GLSource import value_checker, get_posting_sequence, transaction_detail, get_broken_string
from dateutil.relativedelta import relativedelta

HEADER_COLUMN = [280, 330, 200]
COMMON_COLUMN = [35, 5, 35, 5, 55, 5, 270, 5, 262, 5, 55, 5, 55, 15]


class Print_GLFunction:
    ...
    def _header_last_footer(canvas, doc, company_id, issue_from, issue_to):
        ...
        table_data.append(['Prd.', '', 'Source', '', 'Doc.Date', '', 'Description', '', 'Reference', '', 'Debits', '', 'Credits', ''])
        ...
        item_header_table.drawOn(canvas, doc.leftMargin - 7, doc.height + doc.topMargin - h - h1 - 10)
        # Release the canvas
        canvas.restoreState()
            ...
            for i, mAccount in enumerate(account_item_list):
                ...
                for month in range(-1, number_of_month):
                    ...
                    if (len(transaction_item_list_account) or opening_balance) and print_opening_checker:
                        ...
                        table_data.append([mAccount.code, '', '', '', '', '',  mAccount.name, '', 'Opening Balance:', '', db_balance, '', cr_balance, ''])
                        item_table = Table(table_data, colWidths=COMMON_COLUMN)
                        item_table.setStyle(TableStyle(
                            [('INNERGRID', (0, 0), (-1, -1), 0.25, colors.transparent),
                             ('BOX', (0, 0), (-1, -1), 0.25, colors.transparent),
                    ...
                    if transaction_item_list_account:
                        for j, mItem in enumerate(transaction_item_list_account):
                            ...
                            table_data = []
                            transaction_detail_list = transaction_detail(mItem, 48)
                            table_data.append([
                            ...
                            try:
                                desc_list = get_broken_string(transaction_detail_list[2], 48)
                ...
                if sum_amount_debit_acc or sum_amount_credit_acc:
                    table_data = []
                    table_data.append(['', '', '', '', '', '',  '',  '', 'Total: ' + str(mAccount.name)
                                       + ' ' + str(curr_year), '',
                ...
                if print_ending_checker:
                    ...
                    table_data.append(['', '', '', '', '', '',  '',  '', 'Ending Balance : ', '', db_balance, '', cr_balance, ''])
                    # table_data.append([])
                    item_table = Table(table_data, colWidths=COMMON_COLUMN)
                    item_table.setStyle(TableStyle(
                        [('INNERGRID', (0, 0), (-1, -1), 0.25, colors.transparent),
