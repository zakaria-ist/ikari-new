...
class GLFunction_XLS:
    ...
    def print_report(self, company_id, issue_from, issue_to, status_type, acc_list, sp_period):
        ...
        worksheet.write(20, 0, 'Account Number/', center_line)
        ...
        if account_item_list:
            for i, mAccount in enumerate(account_item_list):
                ...
                sum_amount_credit_acc = sum_amount_debit_acc = 0
                curr_year = ''
                for month in range(-1, number_of_month):
                    ...
                    if (len(transaction_item_list_account) or opening_balance) and print_opening_checker:
                        ...
                        worksheet.write(printing_row, printing_col + 5, float(
                            db_balance.replace(',', '')), dec_format if is_decimal else num_format)
                        worksheet.write(printing_row, printing_col + 6, float(
                ...
                if sum_amount_debit_acc or sum_amount_credit_acc:
                ...
                if print_ending_checker:
                    ending_balance = opening_balance + (sum_amount_debit_acc - sum_amount_credit_acc)
                    ...
                    worksheet.write(printing_row, printing_col + 5,
                                    db_balance, dec_format if is_decimal else num_format)
                    worksheet.write(printing_row, printing_col + 6,
                                    cr_balance, dec_format if is_decimal else num_format)
                    printing_row += 2
            ...
            worksheet.write(printing_row, printing_col + 5, round_number(
                accumulate_debit_amount), border_top_bot_dec if is_decimal else border_top_bot_num)
            worksheet.write(printing_row, printing_col + 6, round_number(
                accumulate_credit_amount), border_top_bot_dec if is_decimal else border_top_bot_num)
            printing_row += 2
