...
class Print_SL3A01:
    ...
    def _header_footer(canvas, doc, company_id, delivery_from, delivery_to, cpo_list):
        ...
        # 1st row
        header_data = []
        row1_info1 = "SL3A01 Sales / Purchase System"
        ...
        table_header.append(['Reference No.', 'Line', 'Part No.', 'UOM', 'Order Qty', 'Unit Price', 'Amount(ORG)'])
        table_header.append(['Del. Date', '', 'Part Group', '', '', '', ''])
    ...
    def print_report(self, company_id, delivery_from, delivery_to, document_no, supplier_no, customer_po):
        pdfmetrics.registerFont(TTFont('Arial', os.path.join(s.BASE_DIR, "static/fonts/arial.ttf")))
        pdfmetrics.registerFont(TTFont('Arial-Bold', os.path.join(s.BASE_DIR, "static/fonts/arial-bold.ttf")))
        ...
        for i, mItem in enumerate(order_item_list):
            if customer_po_no != mItem.customer_po_no:
                ...
                if total_qty > 0:
                    table_data = []
                    table_data.append(['', '', '', 'Document Total : ', intcomma("%.2f" % total_qty), '', intcomma("%.2f" % total_amount)])
                    table_data.append(['', '', '', '', '', '', ''])
                ...
                table_data.append([
                    ...
                    mItem.supplier.code + '   ' + str(mItem.supplier.name if mItem.supplier.name else ''), '',
