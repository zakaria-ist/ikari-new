...
import datetime
from utilities.common import update_po_address, round_number, get_decimal_place

rowHeights = 13


class Print_PO_Order:
    ...
    def _header_footer(canvas, doc, order_id, print_header, company_id):
        ...
        if order:
            header_data.append(['', order.supplier.name if order.supplier and order.supplier.name else ''])
            row4_info1 = Paragraph(order.supplier.address.replace('\n', '<br />\n')
                                   if order.supplier else '', styles['LeftAlign'])
            header_data.append(['', row4_info1])
            row5_info1 = 'ATTN: '
            if order.supplier:
        ...
        w, h = header_table.wrap(doc.width, doc.topMargin)
        header_table.drawOn(canvas, doc.leftMargin - 12, doc.height + doc.topMargin - h - 70)

        # == == == == == == == == == == == == == =Header Title RIGHT== == == == == == == == == == == == == =
        header_data = []
        if order:
            row3_info2 = 'P/O NO.'
            row3_info3 = ': ' + order.document_number
            header_data.append([row3_info2, row3_info3])

            row4_info2 = 'DATE'
            row4_info3 = ': ' + order.document_date.strftime('%d %b %Y').upper()
            header_data.append([row4_info2, row4_info3])

            row5_info2 = 'TERMS'
            row5_info3 = ': ' + str(order.supplier.term_days) + ' days' if order.supplier else '0' + ' days'
            header_data.append([row5_info2, row5_info3])

            header_data.append(['PAGE', ': '])

        header_table = Table(header_data, colWidths=[60, 140])
        header_table.setStyle(TableStyle(
            [('INNERGRID', (0, 0), (-1, -1), 0.25, colors.transparent),
             ('BOX', (0, 0), (-1, -1), 0.25, colors.transparent),
             ('FONT', (0, 0), (-1, -1), s.REPORT_FONT),
        ...
        title_table.drawOn(canvas, doc.leftMargin - 21, doc.height + doc.topMargin - h - 180)
        # Release the canvas
        canvas.restoreState()

    def print_report(self, order_id, print_header, company_id, address, remove_address, signature, part_group):
        pdfmetrics.registerFont(TTFont('Arial', os.path.join(s.BASE_DIR, "static/fonts/arial.ttf")))
        pdfmetrics.registerFont(TTFont('Arial-Bold', os.path.join(s.BASE_DIR, "static/fonts/arial-bold.ttf")))
        pdfmetrics.registerFont(TTFont('Arial-Italic', os.path.join(s.BASE_DIR, "static/fonts/arial-italic.ttf")))

        buffer = self.buffer
        doc = SimpleDocTemplate(buffer, rightMargin=42, leftMargin=60, topMargin=260, bottomMargin=42, pagesize=self.pagesize)
        styles = getSampleStyleSheet()
        ...
        if int(remove_address):
            update_po_address(order_id)
        else:
            if int(address):
                try:
                    delivery_info = Delivery.objects.get(is_hidden=0, id=int(address))
                    update_po_address(order_id, int(address))
                except Exception as e:
                    print(e)
                    delivery_info = None
            else:
                try:
                    delivery_info = OrderDelivery.objects.get(is_hidden=0, order_id=order_id)
        ...
        total_data = []
        total_data.append(['', '', 'TOTAL:', intcomma("%.2f" % round_number(total_quantity)), '', intcomma(decimal_place % round_number(order.subtotal))])

        total_table = Table(total_data, colWidths=[30, 190, 90, 75, 75, 75], rowHeights=rowHeights)
        ...
        doc.build(elements,
                  ...
                  canvasmaker=partial(NumberedPage, adjusted_height=-24, adjusted_width=-80, adjusted_caption=''))
