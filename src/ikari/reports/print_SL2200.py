...
rowHeights = 14
colWidths = [110, 120, 30, 50, 60, 60, 60, 60]
...
class Print_SL2200:
    ...
    def _header_footer(canvas, doc, company_id, part_list, group_list):
        ...
        # 1st row
        header_data = []
        row1_info1 = "SL2200 Sales / Purchase System"
        row1_info2 = "Part Sales File Listing"
        ...
        table_header = []
        table_header.append(['Part No.', 'Description', '', '', 'Part Group', 'Std Curr', 'Std Price', 'Last Updated'])
        table_header.append(['Country Origin', 'Short Description', '', '', 'Sales M\'ment', 'Inv M\'ment', 'Rep M\'ment', 'Ratio'])
        table_header.append(['Cust. Code & Name', '', 'Curr', 'Effect Date', 'New Price', 'Sales Price', 'Lead', 'Date Updated'])
    ...
    def print_report(self, company_id, part_code, part_group, customer_no):
        pdfmetrics.registerFont(TTFont('Arial', os.path.join(s.BASE_DIR, "static/fonts/arial.ttf")))
        pdfmetrics.registerFont(TTFont('Arial-Bold', os.path.join(s.BASE_DIR, "static/fonts/arial-bold.ttf")))
        ...
        # if cust_items.count() > 7000:
        #     cust_items = cust_items[:7000]
        for mItem in cust_items.iterator():
            table_data = []
            table_data.append([
                mItem.item.code, mItem.item.name[:40] if mItem.item.name else '', '', '', mItem.item.category.code if mItem.item.category else '',
                mItem.currency.code if mItem.currency else mItem.item.sale_currency.code if mItem.item.sale_currency else '',
                intcomma("%.5f" % mItem.item.sale_price), mItem.item.update_date.strftime("%d/%m/%Y") if mItem.item.update_date else ' / / '])
            table_data.append([
                mItem.item.country.code if mItem.item.country else '', mItem.item.short_description, '', '', 
                mItem.item.inv_measure.code if mItem.item.inv_measure else '',
                mItem.item.inv_measure.code if mItem.item.inv_measure else '', 
                mItem.item.report_measure.code if mItem.item.report_measure else '', 
                intcomma("%.8f" % mItem.item.ratio) if mItem.item.ratio else '1.00000000'])
            ...
            table_data = []
            table_data.append([
                mItem.customer.code + '   ' + mItem.customer.name[:32] if mItem.customer else '', 
                '', mItem.currency.code if mItem.currency else '',
                mItem.effective_date.strftime("%d/%m/%Y") if mItem.effective_date else ' / / ',
                intcomma("%.5f" % mItem.new_price if mItem.new_price else 0),
                intcomma("%.5f" % mItem.sales_price if mItem.sales_price else 0),
                mItem.leading_days, mItem.update_date.strftime("%d/%m/%Y") if mItem.update_date else ' / / '])
        ...
        doc.build(elements, onFirstPage=partial(self._header_footer, company_id=company_id, part_list=part_list, group_list=group_list),
