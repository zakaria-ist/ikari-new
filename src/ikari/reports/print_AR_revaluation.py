...
class Print_AR_revaluation:
    ...
    def _header_footer(canvas, doc, company_id, from_posting, to_posting):
        ...
        # # 2nd row
        row2_info1 = "A/R Revaluation Posting Journal (ARRVPJ1Z)"
        row2_info2 = ""
    ...
    def print_report(self, company_id, from_posting, to_posting):
        ...
        doc = SimpleDocTemplate(buffer, rightMargin=s.REPORT_RIGHT_MARGIN, leftMargin=s.REPORT_LEFT_MARGIN,
                                topMargin=80, bottomMargin=30, pagesize=landscape(A4))
        ...
        elements = []
        table_data = []
        table_header = ['Customer No.', '', 'Document No.', '', 'Date', '', 'Source Amount', '', 'Prior Rate',
        ...
        posting_number = RevaluationLogs.objects.filter(is_hidden=0, company_id=company_id,
                                                        journal_type=dict(TRANSACTION_TYPES)['AR Invoice'], posting_sequence__in=posting_array
                                                        ).exclude(posting_sequence='0').order_by('posting_sequence', 'currency__code')
        ...
        for posting in posting_number:
            ...
            if m_posting_seq != posting.posting_sequence:
                if line_count >= MAX_LINE - 4:
                    ...
                    line_count = 0
                table_data = []
                table_data.append(['Posting Sequence Number:', '', posting.posting_sequence, '',
                                   'Posted On:', '', posting.revaluation_date.strftime('%d/%m/%Y'), '', ''])
            ...
            for mItem in rv_journal_list:
                if m_cust == '':
                    # new cust
                    table_data = []
                    table_data.append([mItem.customer.code if mItem.customer else '', '', mItem.customer.name if mItem.customer else '', '', '', '', '', '', '', '', '', '', '', '', ''])
                    ...
                    if line_count >= MAX_LINE:
                        elements = go_next_page(elements)
                        line_count = 0
                    m_cust = mItem.customer.code if mItem.customer else ''
                if m_cust != '' and m_cust != mItem.customer.code:
                    # prev vendor total
                    table_data = []
                    ...
                    table_data.append(['', '', 'Vendor Total: (' + m_cust + ')', '', '', '', intcomma(decimal_place_f % round_number(doc_total)), '',
                ...
                gl_journal = Journal.objects.filter(journal_type=dict(TRANSACTION_TYPES)['GL'], is_hidden=0, company_id=company_id,
                                                    posting_date=posting.revaluation_date, source_type='AR-GL', name=name,
                ...
                if gl_journal:
                    ...
                    for trx in gl_trx:
                        account_array.append({
                            'code': trx.account.code, 'desc': trx.account.name,
            ...
            table_data.append(['', '', 'Customer Total: (' + m_cust + ')', '', '', '', intcomma(decimal_place % round_number(doc_total)), '',
                                posting.currency.code, '', intcomma(decimal_place_f % round_number(prior_total)), '',
                                intcomma(decimal_place_f % round_number(gain_total)), '', intcomma(decimal_place_f % round_number(new_total))])
            ...
            if not found:
                currency_array.append({
                    'is_decimal': posting.currency.is_decimal, 'curr_code': posting.currency.code,
                    'curr_name': posting.currency.name, 'curr_doc_total': curr_doc_total,
                    'curr_prior_total': curr_prior_total, 'curr_gain_total': curr_gain_total,
                    'curr_new_total': curr_new_total
                })

            table_data = []
            table_data.append([str(cust_count) + ' Customers printed', '', '', '', '', '', ''])
            table_data.append(['', '', '', '', '', '', ''])
        ...
        for m_currency in currency_array:
            if not m_currency['is_decimal']:
                ...
                decimal_place = "%.2f"
            table_data = []
            table_data.append([m_currency['curr_code'], '', m_currency['curr_name'], '', intcomma(decimal_place % round_number(m_currency['curr_doc_total'])),
        ...
        table_data = []
        table_data.append(['', '', '', '', 'Total:', '', intcomma(decimal_place_f % round_number(prior_total)),
                           '', intcomma(decimal_place_f % round_number(gain_total)), '', intcomma(decimal_place_f % round_number(new_total))])
        ...
        for m_acc in account_array:
            ...
            if m_code != m_acc['code'] or m_curr != m_acc['curr']:
                table_data = []
                table_data.append([m_code, '', m_desc, '', m_curr, '', intcomma(decimal_place_f % round_number(m_debit)) if float(m_debit) else '',
                                   '', intcomma(decimal_place_f % round_number(m_credit)) if float(m_credit) else ''])
