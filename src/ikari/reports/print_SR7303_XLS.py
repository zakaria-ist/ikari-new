...
class Print_SR7303_XLS:
    def __init__(self, buffer):
        self.buffer = buffer

    def WriteToExcel(self, company_id, issue_from, issue_to, wanted_from, wanted_to, supplier_code, document_no, customer_po, part_no, sort_by):
        return self.po_issue_report(company_id, issue_from, issue_to, wanted_from, wanted_to, supplier_code, document_no, customer_po, part_no, sort_by)

    def po_issue_report(self, company_id, issue_from, issue_to, wanted_from, wanted_to, supplier_code, document_no, customer_po, part_no, sort_by):
        output = self.buffer
        ...
        worksheet.write(row, 1, 'MSUP_SUPNM', left_bold)
        worksheet.write(row, 2, 'MSUP_SUPCR', left_bold)
        worksheet.write(row, 3, 'PPOH_TRNCD', left_bold)
        worksheet.write(row, 4, 'PPOH_DOCNO', left_bold)
        worksheet.write(row, 5, 'PPOH_DOCDT', right_bold)
        worksheet.write(row, 6, 'PPOD_PARTNO', left_bold)
        worksheet.write(row, 7, 'MPTH_IDESC', left_bold)
        worksheet.write(row, 8, 'PPOD_WANTD', right_bold)
        worksheet.write(row, 9, 'PPOD_CUSTPO', left_bold)
        worksheet.write(row, 10, 'PPOD_LINE', right_bold)
        worksheet.write(row, 11, 'PPOD_REFNO', right_bold)
        worksheet.write(row, 12, 'PPOD_REFLN', right_bold)
        worksheet.write(row, 13, 'PPOH_XRATE', right_bold)
        worksheet.write(row, 14, 'PPOD_QTY', right_bold)
        worksheet.write(row, 15, 'PPOD_UPRICE', right_bold)
        ...
        cust_list = eval(supplier_code)
        if len(cust_list):
            order_item_list = order_item_list.filter(supplier_id__in=cust_list)
        ...
        for i, mItem in enumerate(order_item_list):
            worksheet.write(row, 0, mItem.supplier.code if mItem.supplier_id else '', left_line)
            worksheet.write(row, 1, mItem.supplier.name if mItem.supplier_id and mItem.supplier.name else '', left_line)
            worksheet.write(row, 2, mItem.order.currency.code if mItem.order.currency_id else '', left_line)
            worksheet.write(row, 3, 'P/O', left_line)
            ...
            worksheet.write(row, 11, str(mItem.refer_number), right_line)
            worksheet.write(row, 12, str(mItem.refer_line), right_line)
            worksheet.write(row, 13, round_number(mItem.order.exchange_rate), rate_format)
            worksheet.write(row, 14, mItem.quantity if mItem.quantity else 0.00, dec_format)
            worksheet.write(row, 15, mItem.price if mItem.price else 0.00000, price_format)
            row += 1
