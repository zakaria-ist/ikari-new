...
class GLFunction_Balance_XLS:
    ...
    def print_report(self, company_id, issue_from, issue_to, status_type, acc_list, sp_period):
        ...
        if account_item_list:
            for i, mAccount in enumerate(account_item_list):
                ...
                for month in range(-1, number_of_month):
                    ...
                    if transaction_item_list_account:
                        for j, mItem in enumerate(transaction_item_list_account):
                            ...
                            worksheet.write(printing_row, printing_col + 5, round_number(mItem.functional_amount)
                        ...
                        if (trx_printed and j == transaction_item_list_account.__len__() - 1):
                            ...
                            worksheet.write(printing_row, printing_col + 6, 'Net Change and Ending Balance for Fiscal Period ' +
                ...
                if sum_amount_credit_acc != 0 or sum_amount_debit_acc != 0:
                    worksheet.write(printing_row, printing_col + 4, 'Total: ' + str(mAccount.name) + ' ' + str(curr_year), right_bold)
                    ...
                    worksheet.write(printing_row, printing_col + 7, round_number(sum_amount_debit_acc -
                                    sum_amount_credit_acc), border_top_bot_dec if is_decimal else border_top_bot_num)
                    worksheet.write(printing_row, printing_col + 8, round_number(
        ...
        printing_row += 1
        worksheet.write(printing_row, printing_col + 4, 'Report Total: ', right_bold)
        worksheet.write(printing_row, printing_col + 5, round_number(accumulate_debit_amount),
                        border_top_bot_dec if is_decimal else border_top_bot_num)
        worksheet.write(printing_row, printing_col + 6, round_number(accumulate_credit_amount),
                        border_top_bot_dec if is_decimal else border_top_bot_num)
        worksheet.write(printing_row, printing_col + 7, round_number(g_net),
                        border_top_bot_dec if is_decimal else border_top_bot_num)
        worksheet.write(printing_row, printing_col + 8, round_number(grandtotal),
                        border_top_bot_dec if is_decimal else border_top_bot_num)
        printing_row += 2
