...
class Print_SR7303:
    ...
    def _header_footer(canvas, doc, wanted_from, wanted_to, issue_from, issue_to, pt_list, doc_list, cust_list, po_list, company_id, sort_by):
        ...
        # 1st row
        header_data = []
        row1_info1 = "SR7303 Sales and Purchase System"
        row1_info2 = ""
        row1_info3 = ""
        row1_info4 = "PURCHASE ORDER ISSUED REPORT"
        ...
        row2_info3 = ""
        if sort_by == 'supplier':
            row2_info4 = "Sorting Order: [SUPPLIER CODE]"
        ...
        if len(cust_list):
            row5_info1 = "Supplier Code : [ " + cust_list[0] + " ] - [ " + cust_list[-1] + " ]"
        else:
            row5_info1 = "Supplier Code : [] - []"
        if len(po_list):
        ...
        table_data = []
        # # 1ST ROW
        table_header = ['Supplier Code', 'Curr', 'Exch. Rate', 'Doc.Date', 'Trn', 'Document No./Ln', 'Part No.',
                        'Del. Date', 'Order Qty', 'Unit Price', 'Amount(0)']
        table_data.append(table_header)
        # 2ND ROW
        table_header = ['Supplier Name', '', '', '', '', 'Customer PO No.', 'Part Description', '', '', '', '']
        table_data.append(table_header)
    ...
    def print_report(self, company_id, issue_from, issue_to, wanted_from, wanted_to, supplier_code, document_no, customer_po, part_no, sort_by):
        pdfmetrics.registerFont(TTFont('Arial', os.path.join(s.BASE_DIR, "static/fonts/arial.ttf")))
        pdfmetrics.registerFont(TTFont('Arial-Bold', os.path.join(s.BASE_DIR, "static/fonts/arial-bold.ttf")))
        ...
        if len(cust_list):
            ...
            cust_list = [order_item_list.filter(supplier_id=cust_list[0]).first().order.supplier.code, 
                        order_item_list.filter(supplier_id=cust_list[-1]).first().order.supplier.code]
        ...
        for i, mItem in enumerate(order_item_list):
            ...
            table_data.append(
                [mItem.supplier.code if mItem.supplier_id else '',
                    ...
                    intcomma("%.8f" % round_number(mItem.order.exchange_rate)),
                    mItem.order.document_date.strftime("%d/%m/%Y") if mItem.order.document_date else ' / / ',
                    'P/O',
                    mItem.order.document_number + '   ' + str(mItem.line_number),
                    mItem.item.code if mItem.item_id else '',
            ...
            table_data.append([mItem.supplier.name if mItem.supplier and mItem.supplier.name else '', '', '', '', '',
