...
from accounting.views import Batch
...
COMMON_COLUMN = [30, 5, 30, 5, 50, 5, 175, 5, 175, 5, 25, 5, 65, 5, 65, 5, 65, 5, 65, 3]


class Print_GLSource:
    ...
    def _header_last_footer(canvas, doc, company_id, issue_from, issue_to):
        ...
        w, h = header_table.wrap(doc.width, doc.topMargin)
        header_table.drawOn(canvas, doc.leftMargin - 7, doc.height + doc.topMargin - h - 16)
        ...
        table_data.append([
            ...
            '', '--------- Function Currency ---------', '', ''])
        table_data.append([
            'Prd.', '', 'Source', '', 'Doc. Date', '', 'Description', '', 'Reference', '', 'Curr', '',
        ...
        item_header_table.setStyle(TableStyle(
            [('INNERGRID', (0, 0), (-1, -1), 0.25, colors.transparent),
             ...
             ('SPAN', (-5, -2), (-3, -2)),
             ('SPAN', (-9, -2), (-7, -2)),
             ('ALIGN', (-5, -2), (-3, -2), 'CENTER'),
             ('ALIGN', (-9, -2), (-7, -2), 'CENTER'),
        ...
        item_header_table.drawOn(canvas, doc.leftMargin - 1, doc.height + doc.topMargin - h - h1 - 16)
        # Release the canvas
        canvas.restoreState()
    ...
    def print_report(self, company_id, issue_from, issue_to, status_type, line_1, line_2, line_3, acc_list, sp_period):
        ...
        table_data.append([
            ...
            '', '--------- Function Currency ---------', '', ''])
        table_data.append([
            'Prd.', '', 'Source', '', 'Doc. Date', '', 'Description', '', 'Reference', '', 'Curr', '',  'Debits', '', 'Credits', '', 'Debits', '', 'Credits'])
        ...
        if account_item_list:
            for i, mAccount in enumerate(account_item_list):
                ...
                if open_account_history:
                    ...
                    closing_balance = open_account_history.functional_end_balance
                ...
                for month in range(-1, number_of_month):
                    ...
                    transaction_item_list_account = transaction_item_list.filter(account_id=mAccount.id, journal__perd_year=curr_year,
                        ...
                        .exclude(Q(journal__error_entry__gt=0) & ~Q(journal__source_type='GL-RV'))
                    ...
                    table_data.append([
                        mAccount.code, '', '', '', mAccount.name, '', '', '', 'Opening Balance: ', '', '',
                    ...
                    if transaction_item_list_account:
                        for j, mItem in enumerate(transaction_item_list_account):
                            ...
                            table_data = []
                            transaction_detail_list = transaction_detail(mItem, 35)
                            ...
                            table_data.append([
                                ...
                                mItem.journal.document_date.strftime("%d/%m/%Y") if mItem.journal else ' / / ',
                                '', transaction_detail_list[1], '', transaction_detail_list[0], '',
                                mItem.currency.code if mItem.currency_id else '', '',
                                ...
                                total_f_amnt if mItem.is_debit_account else '', '',
                                total_f_amnt if mItem.is_credit_account else '', '', '', ''])
                            try:
                                table_data.append([
                                    '', '', '', '', '', '', transaction_detail_list[2][:35], '', '', '',
                                    '', '', '', '', '', '', '', '', '', '', '', ''])
                            except:
                                pass
                ...
                table_data = []
                db_balance = intcomma("%.2f" % round_number(closing_balance) if closing_balance > 0 else '0.00')
                cr_balance = intcomma("%.2f" % round_number(abs(closing_balance)) if closing_balance < 0 else '0.00')
                if checker and not db_balance and not cr_balance:
                    db_balance = '0.00'

                table_data.append([
                    '', '', '', '', '', '', '', '', 'Ending Balance: ', '', '', '', '', '', '', '', db_balance, '', cr_balance, '', '', ''])
                table_data.append([])
                item_table = Table(table_data, colWidths=COMMON_COLUMN)
                item_table.setStyle(TableStyle(
                    [('INNERGRID', (0, 0), (-1, -1), 0.25, colors.transparent),
        ...
        table_data.append(['', '', '', '', '', '', '', '', 'Report Total:', '', '', '', '', '', '',
                           ...
                           intcomma("%.2f" % round_number(accumulate_credit_amount)), '', '', ''])
        ...
        doc.build(elements,
                  onFirstPage=partial(self._header_footer, company_id=company_id, issue_from=issue_from, issue_to=issue_to),
...
def value_checker(mAccount, opening_balance):
    db_balance = '0'
    cr_balance = '0'
    if int(mAccount.balance_type) == int(BALANCE_TYPE_DICT['Debit']):
        db_balance = intcomma("%.2f" % round_number(opening_balance))

    if int(mAccount.balance_type) == int(BALANCE_TYPE_DICT['Credit']):
        cr_balance = intcomma("%.2f" % round_number(abs(opening_balance)))

    if int(mAccount.balance_type) == 2 and not cr_balance:
        cr_balance = '0'
    elif not db_balance and not cr_balance:
        db_balance = '0'

    return db_balance, cr_balance


def get_posting_sequence(company_id):
    last_batch = Batch.objects.filter(is_hidden=0, company_id=company_id, batch_type=5
                                      ).annotate(int_posting_sequence=RawSQL('CAST(posting_sequence AS UNSIGNED)', params=[])
                                                 ).order_by('int_posting_sequence').last()
    posting_sequence = None
    if last_batch and last_batch.posting_sequence:
        posting_sequence = last_batch.posting_sequence

    return posting_sequence


def transaction_detail(mItem, length=None):
    if length:
        if len(mItem.description) > length:
            return [mItem.reference[:length] if mItem.reference else '', mItem.description[:length],  mItem.description[length:]]
        else:
            return [mItem.reference[:length] if mItem.reference else '', mItem.description[:length] if mItem.description else '']
    else:
        return [mItem.reference if mItem.reference else '', mItem.description if mItem.description else '']


def get_broken_string(mItem, length=1):
    str_list = []
    if len(mItem) <= length:
        return [mItem]
    while len(mItem) > length:
        str_list.append(mItem[:length])
        mItem = mItem[length:]
    return str_list
