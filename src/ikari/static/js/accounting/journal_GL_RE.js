...
function getTransactionForm() {
    ...
    if($("#txtdesc").val() != '') {
        transaction.description = $("#txtdesc").val();
    ...
    if($("#curr_code").val() != '') {
        transaction.currency_id = $('#curr_id').val();
...
function editOldTransactionModal(id, line) {
    ...
    if (result.length == 0) {
        $.ajax({
            method: "POST",
            url: '/accounting/get-re-detail-transaction/',
            data: {
                'csrfmiddlewaretoken': $('input[name="csrfmiddlewaretoken"]').val(),
                'transaction_id': id
            },
            success: function (data) {
                ...
                transaction.currency_name = data[0].currency_name;
                transaction.exchange_rate = float_format(data[0].exchange_rate).toFixed(10);
                transaction.remark = data[0].remark;
                transaction.rate_date = data[0].rate_date;
...
function deleteOldTransaction(p1,p2,p3,p4){
    if (p2){
        var trxdebits = parseFloat($("#txttrxdebits").val())-parseFloat(p3);
        $("#txttrxdebits").val(trxdebits.toFixed(2));
    } else {
        var trxcredits = parseFloat($("#txttrxcredits").val())-parseFloat(p3);
        $("#txttrxcredits").val(trxcredits.toFixed(2));
    }
    var trxunbalance = Math.abs(parseFloat($("#txttrxdebits").val())-parseFloat($("#txttrxcredits").val()));

    var txttrxdebits = $('#txttrxdebits').val();
    var int_txttrxdebits= txttrxdebits.replace(/,/g , '');
    var txttrxcredits = $('#txttrxcredits').val();
    var int_txttrxcredits = txttrxcredits.replace(/,/g ,'');
    var undistributed_amount = parseFloat(int_txttrxdebits) - parseFloat(int_txttrxcredits);
    $('#txtunbalance').val(undistributed_amount.toFixed(2));
...
$('#entry_num_sub').on('click', function(){
    ...
    $("#div_journal_errorr").addClass('hide_column');
    var form_state = parseInt($('#input_state').val());
    var unbalance = parseFloat($("#txtunbalance").val()).toFixed(2);
    var trxdebits = parseFloat($('#txttrxdebits').val()).toFixed(2);
    var trxcredits = parseFloat($('#txttrxcredits').val()).toFixed(2);
    ...
    if (trx_num>1){
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#txttrxdebits").val(trx.debitsum.toFixed(2));
                $("#txttrxcredits").val(trx.creditsum.toFixed(2));

                var txttrxdebits = $('#txttrxdebits').val();
                int_txttrxdebits= txttrxdebits.replace(/,/g , '');
                var txttrxcredits = $('#txttrxcredits').val();
                int_txttrxcredits = txttrxcredits.replace(/,/g ,'');
                var undistributed_amount = parseFloat(int_txttrxdebits) - parseFloat(int_txttrxcredits);
                $('#txtunbalance').val(undistributed_amount.toFixed(2));
...
function updateTrxDataTables(data){
    ...
    $("#year_period").val(data.journal_prd).trigger('change');
    ...
    for (var i = 0, len = data.array.length; i < len; i++) {
        ...
        transaction.reference = data.array[i].reference;
        transaction.description = data.array[i].description;
        transaction.account_id = data.array[i].account_id;
        transaction.account_code = data.array[i].account_code;
        transaction.account_name = data.array[i].account_name;
        ...
        transaction.exchange_rate = parseFloat(data.array[i].exchange_rate);
        transaction.remark = data.array[i].remark;
        transaction.is_debit_account = data.array[i].is_debit_account;
        if (transaction.is_debit_account==1){
            transaction.srcdebit = parseFloat(data.array[i].total_amount).toFixed(2);
            transaction.srccredit = '0.00';
        } else {
            transaction.srcdebit = '0.00';
            transaction.srccredit = parseFloat(data.array[i].total_amount).toFixed(2);
        }
        transaction.functional_balance_type = data.array[i].functional_balance_type; 
        if (transaction.functional_balance_type=='1'){
            transaction.funcdebit = parseFloat(data.array[i].functional_amount).toFixed(2);
            transaction.funccredit = '0.00';
        } else {
            transaction.funcdebit = '0.00';
            transaction.funccredit = parseFloat(data.array[i].functional_amount).toFixed(2);
        }

        var line = parseInt(datatbl.page.info().recordsTotal) + 1;
        var button = '<div class="btn-group dropup">'
            + '<button type="button" class="btn btn-primary btn-sm dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="min-width: 40px!important;">'
            + '<span class="caret"></span><span class="sr-only">Toggle Dropdown</span>'
            + '</button>'
            + '<ul class="dropdown-menu dropdown-menu-right">';
        if (form_state<3){
            button += '<li><a onclick="editOldTransactionModal('+ transaction.id +','+ line + ')">Edit</a></li>'
            + '<li><a onclick="deleteOldTransactionModal(' + transaction.id +','+ transaction.is_debit_account +','+ parseFloat(data.array[i].functional_amount).toFixed(2) +','+ data.journal_id + ')">Delete</a></li>'
            + '</ul>'
            + '</div>';
        } else {
        ...
        datatbl.row.add( [
            ...
            transaction.account_code+' - '+transaction.account_name,
            transaction.currency_code,
            transaction.srcdebit,
            transaction.srccredit,
            transaction.exchange_rate,
            transaction.funcdebit,
            transaction.funccredit,
            transaction.remark,
            button,
            transaction.id,
            transaction.account_id,
        ...
        trx.debitsum += parseFloat(transaction.funcdebit);
        trx.creditsum += parseFloat(transaction.funccredit);
    }
...
function addOrEditTrx(isAdd,batch_id,journal_id,journal_num){
    ...
    table.rows().every(function (rowIdx, tableLoop, rowLoop) {
        ...
        transaction_list.reference = rowData[$("#trs-reference").index()];
        transaction_list.description = rowData[$("#trs-description").index()];
        transaction_list.srcdebit = parseFloat(rowData[$("#trs-srcdebit").index()]).toFixed(2);
        transaction_list.srccredit = parseFloat(rowData[$("#trs-srccredit").index()]).toFixed(2);
        transaction_list.exchange_rate = parseFloat(rowData[$("#trs-exchange_rate").index()]).toFixed(2);
        transaction_list.funcdebit = parseFloat(rowData[$("#trs-funcdebit").index()]).toFixed(2);
        transaction_list.funccredit = parseFloat(rowData[$("#trs-funccredit").index()]).toFixed(2);
        transaction_list.comment = rowData[$("#trs-comment").index()];
        transaction_list.id = rowData[$("#trs-id").index()];
        transaction_list.account_id = rowData[$("#trs-account_id").index()];
        transaction_list.currency_id = rowData[$("#trs-currency_id").index()];
    ...
    hdrdata.is_auto_reverse = $("#is_auto_reverse").val();
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $(".batch_amount").val(parseFloat(data[0].batch_amount).toFixed(2));
            setHeaderData($("#id_name").val(),$("#document_date").val(),$("#txtsource_code").val());
            transaction_new = [];
            transaction_update = [];
        },
...
function load_unposted_batch(){
    var unposted_batch_dttbl = $('#unposted_batch_table').DataTable();
    unposted_batch_dttbl.destroy();
    unposted_batch_dttbl.clear().draw();
