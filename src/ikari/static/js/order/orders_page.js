...
if($('#company_is_inventory').val() == 'True') {
    emptyRow = `<tr class="gradeX" data-row_index="0">
                    ...
                    <td>
                        <select id="id_select_item_code-0-item_code" class="form-control select_item_code" name="select_item_code-0-item_code" required="required">
...
var SO_SELECT_INDEX_SUPPLIER = 1;
...
$(document).ready(function () {
    ...
    function loadOrderItemModal(selectedRowId, addNew=false) {
        ...
        if (!addNew) {
            ...
            current_code = $($selects[SO_SELECT_INDEX_ITEM_CODE]).val();
            ...
            $('#modal_quantity').off('keyup').on('keyup', function() {
                ...
                if (float_format($('#modal_quantity').val()) < 0) {
                    ...
                    try{
                        var selected_item = $('#modal_part_item_code select').find('option:selected').data('code_data')[0];
            ...
            $('#modal_customer_po_no').off('change').on('change', function () {
                ...
                if ($($cust_po_no).val() && !is_local_check) {
                    var data = {
                        'csrfmiddlewaretoken': $('input[name="csrfmiddlewaretoken"]').val(),
                        'cust_po_no': cuspo,
                        'item_id': $('#modal_part_item_code select').val(),
                        'type': 'SO',
            ...
            $('#modal_part_item_code select').on("select2:select", function() {
                ...
                cusitem = $('#modal_part_item_code select').select2('data')[0].text;
                cur_sup = $('#modal_supplier select').select2('data')[0].text;
                if ($('#company_is_inventory').val() == 'True') {
                    has_location = true;
                    cur_loc = $('#modal_location select').select2('data')[0].text;
                    ...
                    if (cpo != '' && cusitem && cusitem == citem &&
                        ...
                        if (has_location) {
                            if (cur_loc == cloc) {
                                ...
                                show_duplicate_part_modal(duplicate_data_list, '#modal_part_item_code select', '#modal_supplier select', has_location);
                            }
                        } else {
                            var duplicate_data_list = [];
                            duplicate_data_list.push([cLabel[SO_LABEL_INDEX_LINE_NUMBER].textContent, 'Current Doc', citem, cpo, csup, '']);
    ...
    function saveOrderItemModal(selectedRowId) {
        ...
        var modal_part_item_code = $('#modal_part_item_code select').val();
        ...
        // $('#id_select_item_code-' + selectedRowId + '-item_code').val(modal_part_item_code).trigger('change');
        $('#id_select_item_code-' + selectedRowId + '-item_code').val(modal_part_item_code);
        $('#id_select_item_code-' + selectedRowId + '-item_code').select2().trigger('change');
        setTimeout(function() {
            $('#id_select_item_code-' + selectedRowId + '-item_code').select2('close');
            // selectedRowId++; // nth-child start with 1 not 0
            var $current_row = editing_row.find('input');
            var $labels = editing_row.find('label');
            var $selects = editing_row.find('select');
...
function handleEventTable() {
    ...
    $('#dynamic-table tr.gradeX').each(function (index, value) {
        ...
        $($cuspoElement).on('change', function (e) {
            ...
            cusitem = $($item_select).select2('data')[0].text;
    ...
    if (code_pressed == 9) {
        ...
        if (rowIndex == rowCount) {
            ...
            setTimeout(() => {
                ...
                $('#id_select_item_code-' + rowIndex + '-item_code').focus();
                $('#id_select_item_code-' + rowIndex + '-item_code').select2('open');
            }, 300);
        } else {
...
function handlePartNumber(tag_id) {
    ...
    $('#' + tag_id).on("select2:opening", function( event ){
        let line_number = $(this).closest('tr').attr('data-line_number');
        var that = this;
        if (!is_copy && check_linked(line_number) == true) {
            setTimeout(() => {
...
function highLightMandatory(rowCheck) {
    if ($('#id_select_item_code-' + rowCheck +'-item_code').val() == '' || $('#id_select_item_code-' + rowCheck +'-item_code').val() == undefined ) {
        $($('#select2-id_select_item_code-' + rowCheck +'-item_code-container').parent('span')[0]).addClass('highlight-mandatory');
    } else {
        $($('#select2-id_select_item_code-' + rowCheck +'-item_code-container').parent('span')[0]).removeClass('highlight-mandatory');
    }
...
function getFirstFieldInvalid(rowCheck) {
    ...
    if ($('#id_select_item_code-' + rowCheck +'-item_code').val() == '') {
        idFirstInvalid = '#id_select_item_code-' + rowCheck +'-item_code';
...
function is_modal_valid() {
        ...
        if ($('#modal_part_item_code select').val() === '' || $('#modal_part_item_code select').val() === null) {
