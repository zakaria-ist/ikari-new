...
var DO_SELECT_INDEX_REF_NUMBER = 0;
var DO_SELECT_INDEX_REFER_LINE = DO_SELECT_INDEX_REF_NUMBER + 1;
if($('#company_is_inventory').val() == 'True') {
    DO_SELECT_INDEX_LOCATION = DO_SELECT_INDEX_REFER_LINE + 1;
    DO_SELECT_INDEX_COUNTRY_CODE = DO_SELECT_INDEX_LOCATION + 1;
} else {
    DO_SELECT_INDEX_COUNTRY_CODE = DO_SELECT_INDEX_REFER_LINE + 1;
}
...
$(document).ready(function () {
    ...
    $(document).on('select2:close', '#id_customer', function (e) {
      $('#copy_date_doc').focus();
    });
    ...
    var callback = function () {
        $.ajax({
            ...
            success: function (json) {
                ...
                // if (isNaN(taxid)) {
                //     $('#id_tax_amount').val(0);
                //     $('#id_total').val(comma_format((float_format($('#id_tax_amount').val()) -
                //         float_format($('#id_discount').val()) +
                //         float_format($('#id_subtotal').val())), decimal_place));
                // } else {
                //     $.ajax({
...
$(document).ready(function () {
    ...
    function removeRow(tthis) {
        ...
        if (refer_number != '' && refer_number != undefined && refer_line != '' && refer_line != undefined) {
            let indx = rfn_exclude_list.indexOf(refer_number+'-'+refer_line);
            if (rfn_ln_exclude_list.indexOf(refer_number+'-'+refer_line) != -1) {
                rfn_ln_exclude_list.splice(indx, 1);
            }
        }
    ...
    function bindOrderModalEvent() {
            ...
            $(storeCopyRefNumberData).each(function (indx, value) {
                if (value.line == line) {
                    if (check_refer_number != value.ref_number && value.ref_number != 'remove' && value.ref_number != '') {
                        ...
                        let indx = rfn_ln_exclude_list.indexOf(value.ref_number+'-'+value.ref_line);
                        rfn_ln_exclude_list.splice(indx, 1);
                    }
                    found = true;
                }
                ...
                $.ajax({
                    ...
                    success: function (json) {
                        ...
                        $.each(json, function (i, item) {
                            ...
                            if (ref_remain_quantity || rfn_ln_exclude_list.indexOf(check_refer_number+'-'+item.refer_line) == -1) {
                                ref_line_count++;
                                allVals.push({
                        ...
                        if (!ref_line_count) {
                            $('#modal_ref_number select option[value="'+ check_refer_number +'"]').remove();
                            // $('#modal_ref_number select').val('').trigger('change');
                            setTimeout(() => {
                                if ($("#doInvoiceItemModal").is(':visible')) {
                        ...
                        setTimeout(() => {
                            ...
                            if (allVals.length == 1) {
                                ...
                                $('#modal_refer_line select').select2('close');
                            } else if ($('#modal_refer_line select option').length == 2) {
        ...
        $('#modal_quantity_do').off("change").on("change", function() {
            ...
            var location_item_quantity;
            var curr_item_id;
            var total_table_quantity = quantity;
            let rowIndex = editing_row.find('label:first').text();
            let check_refer_number = $('#modal_ref_number_select').val();
            let check_refer_line = $('#modal_refer_line_select').val();
            ...
            for(var i=0; i<allVals.length; i++) {
                if(allVals[i].refer_line == check_refer_line) {
                    location_item_quantity = allVals[i].location_item_quantity;
                    curr_item_id = allVals[i].id;
                }
            }
            $('#dynamic-table tr.gradeX').each(function () {
                var rows = $(this).closest('tr').find('input');
                var is_database = $(this).closest('tr').attr('data-is_database');
                if (is_database == '' || is_database == undefined) {
                    if (curr_item_id && rows[DO_ROW_INDEX_ITEM_ID].value && rows[DO_ROW_INDEX_ITEM_ID].value == curr_item_id) {
                        total_table_quantity += float_format(rows[DO_ROW_INDEX_ITEM_QTY].value);
                    }
                }
            })
            if (outstanding_qty == -1 && float_format(quantity) == 0) {
                ...
                if (rfn_ln_exclude_list.indexOf(check_refer_number+'-'+check_refer_line) == -1) {
                    rfn_ln_exclude_list.push(check_refer_number+'-'+check_refer_line);
                }
            } else if (float_format(quantity) > float_format(outstanding_qty) && float_format(outstanding_qty) > 0) {
                pop_ok_dialog("Invalid Item Quantity",
            ...
            } else if (location_item_quantity && (float_format(quantity) > float_format(location_item_quantity))) {
                var w_str = "Sorry, Insufficient Item Stock Quantity. Current Stock Quantity is ("+ location_item_quantity +")."
                WarnQtyExceded(0, this, w_str);
            } else if (location_item_quantity && (float_format(total_table_quantity) > float_format(location_item_quantity))) {
                var w_str = "Sorry, Insufficient Item Stock Quantity. Current Stock Quantity is ("+ location_item_quantity +")."
                WarnQtyExceded(0, this, w_str);
            } else {
                ...
                if (quantity < last_qty) {
                    ...
                    if (check_refer_number != '' && check_refer_line != '') {
                        let idx = rfn_exclude_list.indexOf(check_refer_number+'-'+check_refer_line);
                        if (rfn_ln_exclude_list.indexOf(check_refer_number+'-'+check_refer_line) != -1) {
                            rfn_ln_exclude_list.splice(idx, 1);
                        }
                    }
                }
    ...
    function check_duplicate(row_id) {
        ...
        $('#dynamic-table tr.gradeX').each(function () {
            ...
            try {
                let selects = $(this).closest('tr').find('select');
                ref_number = selects[DO_SELECT_INDEX_REF_NUMBER].value;
                refer_line = selects[DO_SELECT_INDEX_REFER_LINE].value;
            }catch (e) {
    ...
    function resetReferNumber() {
        if ($("#doInvoiceItemModal").is(':visible')) {
            ...
            rfn_exclude_list.splice(idx, 1);
            let indx = rfn_ln_exclude_list.indexOf(refer_number+'-'+refer_line);
            rfn_ln_exclude_list.splice(indx, 1);
...
function handleQuantity() {
    $('#dynamic-table tr.gradeX').each(function () {
        ...
        $($ItemQty).off('change').on('change', function (e) {
            ...
            let id_refer_line = '#' + $($(this).closest('tr').find('select')[DO_SELECT_INDEX_REFER_LINE]).id;
            ...
            var curr_item_id = currentRow[DO_ROW_INDEX_ITEM_ID].value;
            var total_table_quantity = 0;
            $('#dynamic-table tr.gradeX').each(function () {
                var rows = $(this).closest('tr').find('input');
                var is_database = $(this).closest('tr').attr('data-is_database');
                if (is_database == '' || is_database == undefined) {
            ...
            if ((quantity > 0 && check_refer_line == undefined) ||
                ...
                var w_str = "Sorry, Insufficient Item Stock Quantity. Current Stock Quantity is ("+ location_item_quantity +")."
                WarnQtyExceded(0, this, w_str);
            } else if (!old_line && (is_location == 'True') && (float_format(total_table_quantity) > float_format(location_item_quantity))) {
                var w_str = "Sorry, Insufficient Item Stock Quantity. Current Stock Quantity is ("+ location_item_quantity +")."
                WarnQtyExceded(0, this, w_str);
            } else {
                // let rowNumber = 0;
...
//     currentRow = $(this).closest('tr').find('input');
//     $priceElement = '#' + currentRow[DO_ROW_INDEX_PRICE].id; // ID of Price Column
//     $($priceElement).off('change').on('change', function () {
//         currentRow = $(this).closest('tr').find('input');
//         $('#'+currentRow[DO_ROW_INDEX_AMOUNT].id).val(roundDecimal(float_format(currentRow[DO_ROW_INDEX_ITEM_QTY].value) *
//                                                         float_format(currentRow[DO_ROW_INDEX_PRICE].value) *
//                                                         float_format(currentRow[DO_ROW_INDEX_EXCHANGE_RATE].value), decimal_place)).trigger("change");
//         if (currentRow[DO_ROW_INDEX_PRICE].value > 0) { // Check Price Value
...
//             $('#dynamic-table tr.gradeX').each(function () {
//                 $(this).closest('tr').find('input').not(currentRow[DO_ROW_INDEX_PRICE]).attr('disabled', true);
//             });
//             fnDisableButton();
//         }
//     });
...
// $('#dynamic-table tr.gradeX').each(function () {
//     currentRow = $(this).closest('tr').find('input');
//     $amountElement = '#' + currentRow[DO_ROW_INDEX_AMOUNT].id; // Get ID of Amount Column
//     $($amountElement).on('change', function () {
//         currentRow = $(this).closest('tr').find('input');
//         if (currentRow[DO_ROW_INDEX_AMOUNT].value < 0) {
//             $(this).closest('tr').attr('style', 'background-color: red !important');
//             $('#items_error').text('The product ' + currentRow[DO_ROW_INDEX_ITEM_CODE].value + ' must have amount greater than 0');
//             $('#items_error').removeAttr('style');
//             $('#dynamic-table tr.gradeX').each(function () {
//                 $(this).closest('tr').find('input').not(currentRow[DO_ROW_INDEX_AMOUNT]).attr('disabled', true);
//             });
//             fnDisableButton();
//         } else {
//             var subtotal = 0;
...
//                 var currentRow = $(this).find('input');
//                 $(this).closest('tr').find('input').removeAttr('disabled');
//                 subtotal += float_format(currentRow[DO_ROW_INDEX_AMOUNT].value);
//             });
...
// function calculateDOTotal(selector, index_qty, index_price, index_amt) {
//     var subtotal = 0;
//     var total = 0;
...
//         $('#id_formset_item-'+rowCount+'-amount').text(comma_format(amount, decimal_place));
...
$(document).ready(function () {
    ...
    $('#form').submit(function (e) {
        ...
        $('#dynamic-table tr.gradeX:visible').each(function () {
            ...
            var refer_doc = '';
            var refer_line = '';
            if (currentSelect.length == 2 && $('#company_is_inventory').val() == 'True' || currentSelect.length == 1) {
                refer_doc = currentLabel[1].textContent.trim();
                refer_line = currentLabel[2].textContent.trim();
            } else {
                refer_doc = currentSelect[DO_SELECT_INDEX_REF_NUMBER].value;
                refer_line = currentSelect[DO_SELECT_INDEX_REFER_LINE].value;
            }
...
function load_refer_numbers(){
    ...
    $.ajax({
        type: "GET",
        url: "/orders/do_select_so_json/"+id_cus+'/',
        dataType: 'JSON',
        success: function(data){
        if (data.data.length > 0){
...
function highLightMandatory(rowCheck) {
    if ($('#id_select-' + rowCheck +'-ref_number').val() == '' ||
        $('#id_select-' + rowCheck +'-ref_number').val() == null ||
        $('#id_select-' + rowCheck +'-ref_number').val() == undefined) {
...
function getFirstFieldInvalid(rowCheck) {
    var idFirstInvalid = '';
    if ($('#id_select-' + rowCheck +'-ref_number').val() == '' ||
        ($('#id_select-' + rowCheck +'-ref_number').val() == null && $('#id_select-' + rowCheck +'-ref_number').is(':visible'))) {
    ...
    $($selector).each(function () {
        ...
        selects.each(function (selectIndex, selectValues) {
            ...
            if (selectIndex == DO_SELECT_INDEX_REF_NUMBER) {
                ...
                if (!$("#doInvoiceItemModal").is(':visible') && parseInt($('#id_formset_item-TOTAL_FORMS').val()) > 1) {
                    setTimeout(() => {
                        $(this).select2('open');
                    }, 300);
                }
                        ...
                        if(so_number != '0' && so_number != undefined && so_number != null) {
                            $.ajax({
                                ...
                                success: function (json) {
                                    ...
                                    if (!ref_line_count) {
                                        $('#' + this_id + ' option[value="'+ check_refer_number +'"]').remove();
                                        setTimeout(() => {
                                    ...
                                    referLineSelect2(ref_line_id, allVals, so_number, 'DO');

                                    if (!$("#doInvoiceItemModal").is(':visible')) {
                                        setTimeout(function() {
                                            $(selects[DO_SELECT_INDEX_REF_NUMBER]).select2('close');
                                            $(selects[DO_SELECT_INDEX_REFER_LINE]).select2('open');
                                            if (allVals.length == 1) {
                                                $('#' + ref_line_id).val(allVals[0].refer_line).trigger('change');
                                                $(selects[DO_SELECT_INDEX_REFER_LINE]).select2('close');
                            ...
                            // refreshCurrentRow(currentRow, currentLabel);
                            referLineSelect2(ref_line_id, [], '', 'DO');
                        }
                ...
                $(this).on("select2:close", function (event) {
                    $(this).closest('tr').find('select:eq('+DO_SELECT_INDEX_REFER_LINE+')').focus();
                    let rowCheck = parseInt($(this).closest('tr').attr('data-row_index'));
                    highLightMandatory(rowCheck);
                });
            ...
            if (selectIndex == DO_SELECT_INDEX_REFER_LINE) {
                ...
                if($selected == null) {
                    $(this).on("change", function( event ){
                        ...
                        let currentLabel = $(this).closest('tr').find('label');
                        let currentRow = $(this).closest('tr').find('input');
                        let check_refer_number = $(selects[DO_SELECT_INDEX_REF_NUMBER]).val();
                        ...
                        for(var i=0; i<allVals.length; i++) {
                            if(allVals[i].refer_line == refer_line) {
                                ...
                                // sum += float_format(currentRow[DO_ROW_INDEX_AMOUNT].value);
                                // $('#id_subtotal').val(comma_format(roundDecimal(sum, decimal_place), decimal_place));
                                // if (isNaN(sum)) {
                                //     sum = 0;
                                //     $('#id_subtotal').val(0);
                                ...
                                //     sum += float_format($('#id_tax_amount').val());
                                // }
                                // if ($('#id_discount').val()) {
                                //     sum -= float_format($('#id_discount').val());
                                // }
                                // $('#id_total').val(comma_format(roundDecimal(sum, decimal_place), decimal_place));
