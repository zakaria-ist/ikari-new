...
var GR_SELECT_INDEX_LOCATION = 0;
...
$(document).ready(function () {
    ...
    $(document).on('change', '#id_supplier_code', function (e) {
    ...
    $('#btnReferSelect').on('click', function () {
        ...
        if (refer_id) {
            if ($("#orderItemModal").is(':visible')) {
                ...
                part_current_row[GR_ROW_INDEX_REFER_NO].value = refer_number;
                part_current_row[GR_ROW_INDEX_REFER_LINE].value = refer_line;
                part_current_row[GR_ROW_INDEX_REF_ID].value = refer_id;
                $('#'+part_current_row[GR_ROW_INDEX_REFER_NO].id).trigger('change');
                setTimeout(() => {
                    $('#'+part_current_row[GR_ROW_INDEX_REFER_LINE].id).trigger('change');
                }, 600);
            }
...
function modal_quantity_change() {
    ...
    $('#modal_quantity').off('change').on('change', function(e) {
        ...
        if (float_format($('#modal_quantity').val()) > float_format(outstanding_qty) && float_format(outstanding_qty) > -1) {
...
function is_modal_valid() {
    ...
    if ($('#modal_ref_number').val() === '' || $('#modal_ref_number').val() === null) {
        ...
        $('#modal_ref_number').addClass('highlight-mandatory');
        valid = false;
    } else {
...
function loadOrderItemModal_2(selectedRowId) {
    ...
    if ($('#id_formset_item-' + selectedRowId + '-line_number').text()) {
        ...
        $('#modal_ref_number').val(current_code);
        ...
        $('#modal_refer_line').val(current_code_l);
...
function check_duplicate(row_id) {
    ...
    $('#dynamic-table tr.gradeX').each(function () {
        ...
        let ref_number = $('#id_formset_item-' + rowIndex + '-ref_number').val();
        let refer_line = $('#id_formset_item-' + rowIndex + '-refer_line').val();
...
function referNoEvent($referNo) {
    $($referNo).off('change').on('change', function (e) {
        ...
        var hdSupplierId = $('#hdSupplierId').val();
        // let po_number = currentRow[GR_ROW_INDEX_REF_ID].value;
        let refer_number = $(this).val();
        let this_id = this.id;
        let ref_line_id = 'id_formset_item-' + (parseInt($(this).closest('tr').attr('data-row_index'))) + '-refer_line';
        // let found = false;
        let line = $(this).closest('tr').find('label:first').text();
        // $(storeCopyRefNumberData).each(function (indx, value) {
        //     if (value.line == line) {
        //         if (refer_number != value.ref_number) {
...
function referLineEvent($referLine) {
    $($referLine).off('change').on('change', function (e) {
        ...
        // }
        let check_refer_number = $(currentRow[GR_ROW_INDEX_REFER_NO]).val();
...
function handleQuantity() {
    ...
    $('#dynamic-table tr.gradeX').each(function () {
        ...
        $referNo= '#' + currentRow[GR_ROW_INDEX_REFER_NO].id;
        referNoEvent($referNo);

        $referLine = '#' + currentRow[GR_ROW_INDEX_REFER_LINE].id;
        ...
        $($ItemQty).off('change').on('change', function (e) {
            ...
            let id_refer_line = '#' + $($(this).closest('tr').find('input')[GR_ROW_INDEX_REFER_LINE]).id;
            ...
            // } else {
                check_refer_number = $('#id_formset_item-' + rowIndex +'-ref_number').val();
                check_refer_line = $('#id_formset_item-' + rowIndex +'-refer_line').val();
                // old_line = true;
            // }
            ...
            if ((quantity > 0 && check_refer_line == undefined) ||
                ...
                if (quantity == 'NaN' || quantity <= 0) {
                    ...
                    float_format(outstanding_qty) > -1) {
                    WarnQtyExceded(outstanding_qty, this,
                    "Quantity must not be greater than Outstanding Receive Quantity ("+ outstanding_qty +")");
                }
...
function load_refer_numbers(){
    ...
    $.ajax({
        type: "POST",
        url: "/orders/gr_select_all_po_json/"+id_cus+'/',
        dataType: 'JSON',
        data: {
            'csrfmiddlewaretoken': $('input[name="csrfmiddlewaretoken"]').val(),
        ...
        if (rowIndex == rowCount) {
            ...
            if (refer_number == undefined || refer_number == '') {
                setTimeout(() => {
                    copy_refer_number = getRefNumberDO(copy_refer_number, storeCopyRefNumberData);
                    if ( copy_refer_number != '') {
                        $('#id_formset_item-' + (rowIndex) + '-ref_number').val(copy_refer_number).trigger('change');
                    }
                }, 300);
            } else {
                setTimeout(() => {
...
function highLightMandatory(rowCheck) {
    if ($('#id_formset_item-' + rowCheck +'-ref_number').val() == '' || $('#id_formset_item-' + rowCheck +'-ref_number').val() == undefined ) {
    ...
    if ($('#id_formset_item-' + rowCheck +'-refer_line').val() == '' || $('#id_formset_item-' + rowCheck +'-refer_line').val() == undefined ) {
