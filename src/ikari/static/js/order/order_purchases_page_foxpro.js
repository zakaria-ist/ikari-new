                    ...
                    <td>
                        <div class="col-md-12" style="width: 280px; padding: 0px !important">
                            <div class="col-md-9" style="width: 250px; padding: 0px !important">
                                <label id="id_formset_item-0-code_label" class="control-label-item" name="formset_item-0-code_label" style="display: none"></label>
                            ...
                            <div class="col-md-3" style="width: 10px; padding: 0px !important">
                                <button tabindex="-1" type="button" style="" class="btn btn-white fa fa-search search-partno" name="search-partno" data-toggle="modal"></button>
                            </div>
                        </div>
                    </td>
...
// if($('#company_is_inventory').val() == 'True') {
//     var PO_SELECT_INDEX_LOCATION = PO_SELECT_INDEX_REFER_LINE + 1;
// }

var PO_SELECT_INDEX_LOCATION = 0;
...
$(document).on('click', '.search-partno', function () {
    ...
    if ($("#orderItemModal").is(':visible')) {
        if ($('#modal_ref_number').val() || $('#modal_refer_line').val()) {
            show = false;
        }
    } else {
        if ($(inputs[PO_ROW_INDEX_REFER_NO]).val() || $(inputs[PO_ROW_INDEX_REFER_LINE]).val()) {
            show = false;
        }
    }
    if (show) {
...
$('#btnPartSelect').on('click', function () {
    ...
    if (part_id) {
        if ($("#orderItemModal").is(':visible')) {
            ...
            part_current_row[PO_ROW_INDEX_ITEM_ID].value = part_id;
            $('#'+part_current_row[PO_ROW_INDEX_CODE].id).val(part_code).trigger('change');
            setTimeout(() => {
                $('#'+part_current_row[PO_ROW_INDEX_CUSTOMER_PO].id).focus();
            }, 600);
        }
...
$('#btnReferSelect').on('click', function () {
    ...
    if (refer_id) {
        if ($("#orderItemModal").is(':visible')) {
            ...
            part_current_row[PO_ROW_INDEX_REFER_NO].value = refer_number;
            part_current_row[PO_ROW_INDEX_REFER_LINE].value = refer_line;
            part_current_row[PO_ROW_INDEX_REFERENCE_ID].value = refer_id;
            $('#'+part_current_row[PO_ROW_INDEX_REFER_NO].id).trigger('change');
            setTimeout(() => {
                $('#'+part_current_row[PO_ROW_INDEX_REFER_LINE].id).trigger('change');
                $('#'+part_current_row[PO_ROW_INDEX_CUSTOMER_PO].id).focus();
            }, 600);
        }
...
$(document).ready(function () {
    ...
    // $(document).on('select2:close', '#modal_ref_number', function (e) {
    ...
    //         $('#modal_customer_po_no').focus();
    //     } else {
    //         $('#modal_part_item_code').focus();
    //     }
    // });

    $(document).on('blur', '#modal_part_item_code', function (e) {
        $('#modal_customer_po_no').focus();
    });
    ...
    $('#id_supplier_code').on('blur', function (e)
    {
...
$(document).on("input", "#id_supplier_code", function() {
    this.value = this.value.toUpperCase();
});

$('#id_supplier_code').on('change', function() {
    var sup_code = $(this).val();
    $.ajax({
        type: "GET",
        url: "/suppliers/get_by_code/"+sup_code+'/',
        dataType: 'JSON',
        success: function(data){
            if (data.data.length > 0){
                $('#id_supplier').val(data.data[0].id).trigger('change');
            } else {
                $('#dynamic-table tbody').find('tr').remove();
                pop_ok_dialog("Invalid Supplier Code",
                sup_code + " is not found in the system, try again.",
                function () {
                    $('#id_supplier_code').val('');
                    $('#id_supplier_code').focus();
                });
            }
        }
    });
...
$(document).ready(function () {
    ...
    $('#form').submit(function (e) {
        ...
        $('#dynamic-table tr.gradeX:visible').each(function () {
            ...
            var refer_doc = currentRow[PO_ROW_INDEX_REFER_NO].value;
            var refer_line = currentRow[PO_ROW_INDEX_REFER_LINE].value;
...
$(document).ready(function (hdSupplierId) {
    ...
    $('#btnSupplierSelect').on('click', function () {
        ...
        var supplier_select_code = $("input[name='choices']:checked").attr('code');

        $('#hdSupplierId').val(supplier_select_id);
        $('#id_supplier').val(supplier_select_id).trigger('change');
        $('#id_supplier_code').val(supplier_select_code);
...
$(document).ready(function () {
    ...
    $(document).on('click', "[class^=appendrow]", function (event) {
        if (rowComplete()) {
            ...
            if ( copy_refer_number != '') {
                $('#id_formset_item-' + (rowNumber) + '-ref_number').val(copy_refer_number).trigger('change');
            }
    ...
    function bindOrderModalEvent() {
        ...
        $('#modal_ref_number').on("change", function() {
            ...
            if (so_number != null && supp_id != null) {
                $.ajax({
                    ...
                    success: function (json) {
                        ...
                        setTimeout(function() {
                            ...
                            if (allVals.length != 1) {
                                $('#modal_part_item_code').attr("readonly", "readonly");
                            }
                        }, 300);
                    }
                });
                ...
                clear_modal();
                current_code = $('#id_formset_item-' + dyn_tbl_sel_row_id + '-code').val();
                line_object['part_item'] = current_code;
                
            }
        });

        $('#modal_refer_line').on("change", function( event ){
        ...
        $('#modal_part_item_code').on("change", function() {
            let selected = $('#modal_part_item_code').val();
            let item_code = item_info.filter(function (el) {
                return el.code == selected;
            });
            if(item_code.length) {
                item_code = item_code[0];
                // $('#modal_customer_po_no').val(item_code.customer_po_no);
                $('#modal_part_item_code').val(item_code.code);
                ...
                if (selected != '') {
                    pop_ok_dialog("Invalid Part No.",
                        selected + " is not found in the system, try again.",
                        function () {
                        });
                }
                clear_modal();
    ...
    function loadOrderItemModal_2(selectedRowId, addNew=false) {
        ...
        if (!addNew) {
            ...
            $('#modal_part_item_code').val($('#id_formset_item-' + selectedRowId + '-code').val());
            ...
            line_object['part_item'] = $('#modal_part_item_code').val();
    ...
    function is_modal_valid() {
        ...
        if ($('#modal_part_item_code').val() === '' || $('#modal_part_item_code').val() === null) {
    ...
    function check_duplicate(row_id) {
        ...
        $('#dynamic-table tr.gradeX').each(function () {
            ...
            try {
                ...
                ref_number = selects[PO_ROW_INDEX_REF_NO].value;
                refer_line = selects[PO_ROW_INDEX_REFER_LINE].value;
            }catch (e) {
    ...
    function saveOrderItemModal(selectedRowId) {
        let modal_ref_number = $('#modal_ref_number').val();
        let modal_refer_line = $('#modal_refer_line').val();
        let modal_item_code = $('#modal_part_item_code').val();
        ...
        if (modal_ref_number != '') {
            ...
            setTimeout(function() {
                if (modal_refer_line != '') {
                    ...
                    $('#id_formset_item-' + selectedRowId + '-code').val(modal_item_code).trigger('change');
                }
                setTimeout(function() {
        ...
        $('#modal_part_item_code').removeAttr("readonly");
    }
    ...
    function is_change() {
        ...
        if (line_object['part_item'] != $('#modal_part_item_code').val()
            && $('#modal_part_item_code').val() != undefined) {
            flag_change = true;
        }
    ...
    function removeLine() {
        // try{
        //     $('#modal_ref_number').select2('close');
        //     $('#modal_refer_line').select2('close');
...
//     $.getJSON(url_get_item_backorder+'?item_id='+$('#modal_part_item_code').val(), function(data) {
//         var backorder_qty = 0;
...
function handleQuantity() {
    $('#dynamic-table tr.gradeX').each(function () {
        ...
        $referNo = '#' + currentRow[PO_ROW_INDEX_REFER_NO].id;
        referNoEvent($referNo);

        $referLine = '#' + currentRow[PO_ROW_INDEX_REFER_LINE].id;
        referLineEvent($referLine);

        $partNo = '#' + currentRow[PO_ROW_INDEX_CODE].id;
        partNoEvent($partNo);
...
function referNoEvent($referNo) {
    $($referNo).off('change').on('change', function (e) {
        ...
        // let so_number = currentRow[PO_ROW_INDEX_REFERENCE_ID].value;
        let tthis = this;
        // let is_update = false;
        let supp_id = $('#id_supplier').val();
        if(so_number != '0' && so_number != undefined && so_number != null) {
            $.ajax({
                ...
                success: function (json) {
                    ...
                    if (!$("#orderItemModal").is(':visible')) {
                        setTimeout(function() {
                            $("#"+currentRow[PO_ROW_INDEX_CODE].id).attr("readonly", "readonly");
                        }, 300);
                    }
                }
            });
            ...
            refreshCurrentRow(currentRow, currentLabel);
            $("#"+currentRow[PO_ROW_INDEX_CODE].id).removeAttr("readonly");
        }
...
function referLineEvent($referLine) {
    $($referLine).off('change').on('change', function (e) {
        ...
        let currentLabel = $(this).closest('tr').find('label');
        let currentRow = $(this).closest('tr').find('input');
        let check_refer_number = currentRow[PO_ROW_INDEX_REFER_NO].value;
...
function partNoEvent($partNo) {
    $($partNo).off('change').on('change', function (e) {
        let currentLabel = $(this).closest('tr').find('label');
        let currentRow = $(this).closest('tr').find('input');
        let selects = $(this).closest('tr').find('select');
        ...
        if (item_code.length) {
            ...
            currentRow[PO_ROW_INDEX_ITEM_NAME].value = item_code.name;
            currentRow[PO_ROW_INDEX_ITEM_ID].value = item_code.id;
            ...
            if ($('#company_is_inventory').val() == 'True' && item_code.location_list.length){
                ...
                $(location_choice).append(options);
                $(location_choice).select2();
                $(location_choice).val(item_code.location_list[0].id).trigger('change');
            }

            currentRow[PO_ROW_INDEX_ITEM_PRICE].value = float_format(item_code.purchase_price).toFixed(6);
            currentRow[PO_ROW_INDEX_CURRENCY_CODE].value = item_code.currency;
            currentRow[PO_ROW_INDEX_CURRENCY_ID].value = item_code.currency_id;
            currentRow[PO_ROW_INDEX_CATEGORY].value = item_code.category;
            currentRow[PO_ROW_INDEX_SUPPLIER_CODE].value = item_code.supplier;
            currentRow[PO_ROW_INDEX_SUPPLIER_ID].value = item_code.supplier_id;
            currentRow[PO_ROW_INDEX_REFER_NO].value = item_code.ref_no;
            currentRow[PO_ROW_INDEX_REFER_LINE].value = item_code.ref_line;
            currentRow[PO_ROW_INDEX_BACKORDER_QTY].value = item_code.backorder_qty;
            currentRow[PO_ROW_INDEX_UOM].value = item_code.uom;
            currentRow[PO_ROW_INDEX_REFERENCE_ID].value = item_code.ref_id;
            currentRow[PO_ROW_INDEX_MIN_ORDER_QTY].value = item_code.minimun_order;
            ...
            currentLabel[PO_LABEL_INDEX_BACKORDER_QTY].textContent = comma_format(currentRow[PO_ROW_INDEX_BACKORDER_QTY].value); // Back Order Quantity
            currentLabel[PO_LABEL_INDEX_UOM].textContent = currentRow[PO_ROW_INDEX_UOM].value; // UOM
            $(this).removeClass('highLightMandatory');
            ...
            $('#dynamic-table tr.gradeX:nth-child('+row+')').each(function () {
                ...
                selects.each(function (selectIndex, selectValue) {
                    if (selectIndex == PO_SELECT_INDEX_LOCATION) {
                        if ($('#company_is_inventory').val() == 'True') {
                            $(this).find('option[value="' + item_code.location_id + '"]').prop('selected', true);
                        }
                    }
                });
            });
            ...
            pop_ok_dialog("Invalid Part No",
                ...
                function () {
                    $($partNo).val('');
                    $($partNo).focus();
                    $($partNo).addClass('highLightMandatory');
                    refreshCurrentRow(currentRow, currentLabel);
                });
        }
    })
...
function quantityEvent($ItemQty) {
    $($ItemQty).off('change').on('change', function (e) {
        ...
        if (float_format(quantity) < float_format(minoq) && float_format(quantity) > 0) {
            ...
            let rowIndex = parseInt($(this).closest('tr').attr('data-row_index'));
            let check_refer_number = $($(this).closest('tr').find('input')[PO_ROW_INDEX_REFER_NO]).val();
            let check_refer_line = $($(this).closest('tr').find('input')[PO_ROW_INDEX_REFER_LINE]).val();
...
function load_refer_numbers(){
    ...
    $.ajax({
        type: "GET",
        url: "/orders/po_by_so_all_data_json/"+id_cus+'/',
        dataType: 'JSON',
        success: function(data){
        if (data.data.length > 0){
...
function highLightMandatory(rowCheck) {
    ...
    if ($('#id_formset_item-' + rowCheck +'-item_code').val() == '') {
        $('#id_formset_item-' + rowCheck +'-item_code').addClass('highlight-mandatory');
    } else {
        $('#id_formset_item-' + rowCheck +'-item_code').removeClass('highlight-mandatory');
    }
...
function getFirstFieldInvalid(rowCheck) {
    ...
    if ($('#id_formset_item-' + rowCheck +'-item_code').val() == '') {
        idFirstInvalid = '#id_formset_item-' + rowCheck +'-item_code';
        return idFirstInvalid;
    }
