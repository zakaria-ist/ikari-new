...
-- IF THESE BELOW MEASURES DO NOT EXISTS IN DB THEN RUN THIS ONE TIME --
-- OR INSERT THOSE MISSING ONES --
...
LEFT OUTER JOIN (
    SELECT
        iitm_fil.*
        ,(SELECT id FROM items_itemcategory WHERE SUBSTRING(code,1,20) = iitm_itmgr and company_id=@company_id ORDER BY id LIMIT 1) category_id
        ,(SELECT id FROM items_itemmeasure WHERE CASE WHEN iitm_mment='L' THEN code='LITRE' ELSE code=iitm_mment END) inv_measure_id
        ,(SELECT id FROM items_itemmeasure WHERE CASE WHEN iitm_oment='L' THEN code='LITRE' ELSE code=iitm_oment END) sales_measure_id
        ,(SELECT id FROM items_itemmeasure WHERE CASE WHEN iitm_iment='L' THEN code='LITRE' ELSE code=iitm_iment END) purchase_measure_id
...
LEFT OUTER JOIN (
    SELECT
        ...
        ,mpth_ratio -- ratio --
        ,(SELECT id FROM items_itemcategory WHERE SUBSTRING(code,1,20) = mpth_group and company_id=@company_id ORDER BY id LIMIT 1) category_id
        ...
        ,(SELECT id FROM items_itemmeasure WHERE CASE WHEN mpth_mment='L' THEN code='LITRE' ELSE code=mpth_mment END) inv_measure_id
        ,(SELECT id FROM items_itemmeasure WHERE CASE WHEN mpth_oment='L' THEN code='LITRE' ELSE code=mpth_oment END) sales_measure_id
        ,(SELECT id FROM items_itemmeasure WHERE CASE WHEN mpth_iment='L' THEN code='LITRE' ELSE code=mpth_iment END) purchase_measure_id
        ,(SELECT id FROM items_itemmeasure WHERE CASE WHEN mpth_ruom='L' THEN code='LITRE' ELSE code=mpth_ruom END) report_measure_id
...
FROM (
    SELECT msupfil0.*
        ,(CASE
            ...
            WHEN msupfil0.msup_supcr='€' THEN 'EUR'
...
-- remove duplicate --
DELETE t1 FROM `customers_customer` t1
INNER JOIN `customers_customer` t2
WHERE
...
DELETE t1 FROM `suppliers_supplier` t1
INNER JOIN `suppliers_supplier` t2
WHERE
    `t1`.`company_id`=@company_id
    AND `t2`.`company_id`=@company_id
    AND `t1`.`currency_id` = `t2`.`currency_id`
