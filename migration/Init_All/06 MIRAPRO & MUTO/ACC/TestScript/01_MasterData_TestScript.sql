...
BEGIN
  ...
  SELECT count(src.acctset)
    ...
    FROM (
      ...
      LEFT JOIN `accounts_account` AS `ctrl_acc` ON `ctrl_acc`.`code` = REPLACE(`IDACCTAP`, '-', '') AND `ctrl_acc`.`company_id` = I_global_var1
      LEFT JOIN `accounts_account` AS `rlzdgain_acc` ON `rlzdgain_acc`.`code` = REPLACE(`RLZGNACT`, '-', '') AND `rlzdgain_acc`.`company_id` = I_global_var1
      LEFT JOIN `accounts_account` AS `rlzdloss_acc` ON `rlzdloss_acc`.`code` = REPLACE(`RLZLSACT`, '-', '') AND `rlzdloss_acc`.`company_id` = I_global_var1
      LEFT JOIN `accounts_account` AS `rounding_acc` ON `rounding_acc`.`code` = REPLACE(`RNDACCT`, '-', '') AND `rounding_acc`.`company_id` = I_global_var1
      LEFT JOIN `accounts_account` AS `unrlgain_acc` ON `unrlgain_acc`.`code` = REPLACE(`URLZGNACT`, '-', '') AND `unrlgain_acc`.`company_id` = I_global_var1
      LEFT JOIN `accounts_account` AS `unrlloss_acc` ON `unrlloss_acc`.`code` = REPLACE(`URLZLSACT`, '-', '') AND `unrlloss_acc`.`company_id` = I_global_var1
      LEFT JOIN `accounts_account` AS `prepayment_acc` ON `prepayment_acc`.`code` = REPLACE(`PPAYACCT`, '-', '') AND `prepayment_acc`.`company_id` = I_global_var1
      LEFT JOIN `accounts_account` AS `discount_acc` ON `discount_acc`.`code` = REPLACE(`DISCACCT`, '-', '') AND `discount_acc`.`company_id` = I_global_var1
      UNION
      SELECT IDACCTSET
        FROM `ikari_db_sage300`.`arras`
      LEFT JOIN `accounts_account` AS `ctrl_acc` ON `ctrl_acc`.`code` = REPLACE(`ARIDACCT`, '-', '') AND `ctrl_acc`.`company_id` = I_global_var1
      LEFT JOIN `accounts_account` AS `rlzdgain_acc` ON `rlzdgain_acc`.`code` = REPLACE(`RLZDGAIN`, '-', '') AND `rlzdgain_acc`.`company_id` = I_global_var1
      LEFT JOIN `accounts_account` AS `rlzdloss_acc` ON `rlzdloss_acc`.`code` = REPLACE(`RLZDLOSS`, '-', '') AND `rlzdloss_acc`.`company_id` = I_global_var1
      LEFT JOIN `accounts_account` AS `rounding_acc` ON `rounding_acc`.`code` = REPLACE(`RNDACCT`, '-', '') AND `rounding_acc`.`company_id` = I_global_var1
      LEFT JOIN `accounts_account` AS `unrlgain_acc` ON `unrlgain_acc`.`code` = REPLACE(`UNRLGAIN`, '-', '') AND `unrlgain_acc`.`company_id` = I_global_var1
      LEFT JOIN `accounts_account` AS `unrlloss_acc` ON `unrlloss_acc`.`code` = REPLACE(`UNRLLOSS`, '-', '') AND `unrlloss_acc`.`company_id` = I_global_var1
      LEFT JOIN `accounts_account` AS `prepayment_acc` ON `prepayment_acc`.`code` = REPLACE(`CASHLIAB`, '-', '') AND `prepayment_acc`.`company_id` = I_global_var1
      LEFT JOIN `accounts_account` AS `discount_acc` ON `discount_acc`.`code` = REPLACE(`ACCTDISC`, '-', '') AND `discount_acc`.`company_id` = I_global_var1
      LEFT JOIN `accounts_account` AS `writeoff_acc` ON `writeoff_acc`.`code` = REPLACE(`ACCTWROF`, '-', '') AND `writeoff_acc`.`company_id` = I_global_var1) src;
  ...
  -- test16
  CALL generate_accthist_src(I_global_var1,@source_tbl);

  IF @source_tbl THEN
    CALL check_test('test16',test_accthist(I_global_var1),L_error_cnt,L_success_cnt,L_error_msg);
  ELSE
    SET L_error_cnt := L_error_cnt + 1;
