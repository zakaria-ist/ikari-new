...
UPDATE taxes_tax tx set `code` = CONCAT('OG', tx.number) where `name` like '%SALES%' and company_id=@company_id;
UPDATE taxes_tax tx set `code` = CONCAT('IG', tx.number) where `name` like '%PURCHASE%' and company_id=@company_id;
...
`is_active`,`is_hidden`,`create_date`,`update_date`,
