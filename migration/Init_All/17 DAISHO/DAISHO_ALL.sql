...
set account_segment = RIGHT((SELECT SUBSTRING_INDEX(code, "-", 1)), 5) where company_id=@company_id;
...
SELECT
    ...
    ,(select id from accounts_accountset
        where code = IDACCTSET
            and company_id=@company_id
            and type=2 -- ACC_SET_TYPE['AP ACCOUNT SET'] --
     ) account_set_id
     , null bank_id
...
LEFT JOIN accounting_paymentcode pc
  ON j.PAYMCODE = pc.code and pc.source_type='AP' and pc.company_id=@company_id
WHERE j.btchtype = 'PY'
UNION
SELECT j.CNTITEM code
    ,j.INVCDESC name
...
LEFT JOIN customers_customer c
  ON j.IDCUST = c.code AND c.company_id = @company_id
LEFT JOIN accounting_paymentcode pc
  ON j.CODEPAYM = pc.code and pc.source_type='AR' and pc.company_id=@company_id
...
SELECT t1.* FROM (SELECT '1' code
    ...
    ,(SELECT id FROM accounting_paymentcode where code=ar.CODEPAYM
       and source_type='AR' and company_id=@company_id) payment_code_id
    ,NULL payment_currency_id
    ,t.glref reference
    ,NULL source_type
    ,(CASE
