...
DELETE FROM taxes_tax WHERE is_hidden = 1 and company_id=@company_id;
...
create procedure clean_taxes_tax_data()
begin

UPDATE taxes_tax tx set `code` = CONCAT('OG', tx.number) where `name` like '%SALES%' and company_id=@company_id and tx.number is not null;
UPDATE taxes_tax tx set `code` = CONCAT('IG', tx.number) where `name` like '%PURCHASE%' and company_id=@company_id and tx.number is not null;
...
UPDATE taxes_tax tx set `number` = 2 -- TAX_CLASS_DICT['Zero Rate'] --
    where `name` like '%0%' and company_id=@company_id;
UPDATE taxes_tax tx set `number` = 2 -- TAX_CLASS_DICT['Zero Rate'] --
    where `name` like '%Z%' and company_id=@company_id;
UPDATE taxes_tax tx set `number` = 2 -- TAX_CLASS_DICT['Zero Rate'] --
    where `name` like '%FREIGHT%' and company_id=@company_id;
UPDATE taxes_tax tx set `number` = 1 -- TAX_CLASS_DICT['Standard Rate'] --
    where rate <> 0.00 and company_id=@company_id;
UPDATE taxes_tax tx set `number` = 2 -- TAX_CLASS_DICT['Zero Rate'] --
    where `name` like '%EXPORT%' and company_id=@company_id;
UPDATE taxes_tax tx set `number` = 3 -- TAX_CLASS_DICT['Exempted'] --
    where `name` like '%EXEM%' and company_id=@company_id;
UPDATE taxes_tax tx set `number` = 4 -- TAX_CLASS_DICT['OUT OF SCOPE'] --
    where `name` like '%SCOPE%' and company_id=@company_id;

UPDATE taxes_tax tx set `update_date` = '2017-01-01' where `update_date` = '0000-00-00' and company_id=@company_id;

select
	@tax_group_sales_id := id
from taxes_taxgroup tx where company_id=@company_id and transaction_type=1 limit 1;

select
	@tax_group_purchase_id := id
from taxes_taxgroup tx where company_id=@company_id and transaction_type=2 limit 1;

select
	/* @tax_account_code_id := tax_account_code_id, */
	@tax_authority_id := tax_authority_id
from taxes_tax tx where company_id=@company_id limit 1;

UPDATE taxes_tax set
	/* tax_account_code_id = @tax_account_code_id, */
	tax_authority_id := @tax_authority_id,
	tax_group_id := @tax_group_sales_id
where company_id=@company_id and (`name` like '%SALE%' or  `code` like 'O%');

UPDATE taxes_tax set
	/* tax_account_code_id = @tax_account_code_id, */
	tax_authority_id := @tax_authority_id,
	tax_group_id := @tax_group_purchase_id
where company_id=@company_id and (`name` like '%PUR%' or  `code` like 'I%');
...
LEFT OUTER JOIN (
    SELECT
        iitm_fil.*
        ,(SELECT id FROM items_itemcategory WHERE SUBSTRING(code,1,20) = iitm_itmgr and company_id=@company_id) category_id
...
LEFT OUTER JOIN (
    SELECT
        ...
        ,mpth_ratio -- ratio --
        ,(SELECT id FROM items_itemcategory WHERE SUBSTRING(code,1,20) = mpth_group and company_id=@company_id) category_id
    ...
    FROM (SELECT mpth_fil.*
		,(CASE
			...
			WHEN mpth_pcur='BHT' THEN 'BAH'
            WHEN mpth_pcur='RM' THEN 'MYR'
	        ELSE mpth_pcur
	    END) new_mpth_pcur
...
`code`,`name`,`address`,`phone`,`fax`,
`is_hidden`,`is_active`,`create_date`,`update_date`,
`company_id`,`country_id`,`currency_id`,`tax_id`,
`location_id`,`payment_mode_id`,`payment_term`,
...
FROM
    (SELECT mcusfil0.*
        ,(CASE
			...
			WHEN mcusfil0.mcus_cuscr='BHT' THEN 'BAH'
            WHEN mcusfil0.mcus_cuscr='RM' THEN 'MYR'
...
FROM (
    SELECT msupfil0.*
        ,(CASE
            ...
            WHEN msupfil0.msup_supcr='BHT' THEN 'BAH'
            WHEN msupfil0.msup_supcr='RM' THEN 'MYR'
...
create procedure merge_customers(in code1 varchar(20), in code2 varchar(20))
begin
UPDATE customers_customer as t1
    INNER JOIN customers_customer as t2 ON
        (t1.code = code1 AND t2.code = code2 AND t1.company_id = t2.company_id AND t2.company_id = @company_id and t1.id < t2.id)
...
UPDATE customers_customer as t1
    INNER JOIN customers_customer as t2 ON
        (t1.code = t2.code AND t1.company_id = @company_id AND t2.company_id = @company_id and t1.id < t2.id)
...
create procedure merge_suppliers(in code1 varchar(20), in code2 varchar(20))
begin
...
DELETE from suppliers_supplier where company_id=@company_id and code='DELETETHIS';
DELETE from suppliers_supplier where company_id=@company_id and code = '';
end$$
delimiter ;

delimiter $$
DROP PROCEDURE IF EXISTS set_customer_currency$$
create procedure set_customer_currency()
begin
...
where company_id=@company_id and `code` like '%-U' and currency_id is null;

UPDATE customers_customer set
	currency_id := @USD_currency_id
where company_id=@company_id and (`code` like '%US$%' or `name` like '%US$%') and currency_id is null;

-- SET SGD --

UPDATE customers_customer set
	currency_id := @SGD_currency_id
where company_id=@company_id and `code` like '%-S' and currency_id is null;

UPDATE customers_customer set
	currency_id := @SGD_currency_id
where company_id=@company_id and (`code` like '%S$%' or `name` like '%S$%') and currency_id is null;

UPDATE customers_customer set
	currency_id := @SGD_currency_id
where company_id=@company_id and (`name` like '%PTE LTD') and currency_id is null;

UPDATE customers_customer set
	currency_id := @SGD_currency_id
where company_id=@company_id and `code` like '%SGD' and currency_id is null;

-- SET YEN --

UPDATE customers_customer set
	currency_id := @YEN_currency_id
where company_id=@company_id and `code` like '%-Y' and currency_id is null;

end$$
delimiter ;

call set_customer_currency();


delimiter $$
DROP PROCEDURE IF EXISTS set_supplier_currency$$
create procedure set_supplier_currency()
begin
...
call set_supplier_currency();


delimiter $$
DROP PROCEDURE IF EXISTS set_supplier_country$$
create procedure set_supplier_country()
begin
...
where company_id=@company_id and `name` like '%US$%' and country_id is null;

-- SET SIN --
UPDATE suppliers_supplier set
	country_id := @SIN_country_id
where company_id=@company_id and `name` like '%PTE LTD%' and country_id is null;

UPDATE suppliers_supplier set
	country_id := @SIN_country_id
where company_id=@company_id and `name` like '%P/L%' and country_id is null;

UPDATE suppliers_supplier set
	country_id := @SIN_country_id
where company_id=@company_id and `name` like '%(S)PL%' and country_id is null;
...
UPDATE suppliers_supplier set
	country_id := @SIN_country_id
where company_id=@company_id and `name` like '%S$%' and country_id is null;

UPDATE suppliers_supplier set
	country_id := @SIN_country_id
where company_id=@company_id and `name` like '%SINGAPORE%' and country_id is null;

-- SET MAL --

UPDATE suppliers_supplier set
	country_id := @MAL_country_id
where company_id=@company_id and `name` like '%SDN%' and country_id is null;

-- SET THA --

UPDATE suppliers_supplier set
	country_id := @THA_country_id
where company_id=@company_id and `code` like '%BHT%' and country_id is null;

UPDATE suppliers_supplier set
	country_id := @THA_country_id
where company_id=@company_id and `name` like '%THAILAND%' and country_id is null;

-- SET VTN --

UPDATE suppliers_supplier set
	country_id := @VTN_country_id
where company_id=@company_id and `name` like '%VIETNAM%' and country_id is null;


end$$
delimiter ;

call set_supplier_country();



delimiter $$
DROP PROCEDURE IF EXISTS set_customer_country$$
create procedure set_customer_country()
begin
