...
SELECT (
	...
	,null update_by
	,0 is_hidden
FROM ikari_db_foxpro_sp.mptd_fil where mptd_prity='S';
...
SELECT
	...
	,null update_by
	,0 is_hidden
FROM ikari_db_foxpro_sp.mptd_fil where mptd_prity='P';
...
-- delimiter $$
-- DROP PROCEDURE IF EXISTS update_customer_distributioncode$$
-- create procedure update_customer_distributioncode()
-- begin
-- UPDATE customers_customer as cc
-- SET cc.distribution_code_id = (SELECT final.dis_id FROM
--   (SELECT dis_tbl.id as dis_id, accode.code as acc_code FROM accounts_distributioncode as dis_tbl
--   JOIN (SELECT distinct tbl_1.acc_id, tbl_1.code FROM
--     (SELECT t2.id as acc_id, t1.id as cust_id, t2.code as code from customers_customer as t1
--       JOIN accounts_account as t2 on t1.accode_sal = t2.code where t1.company_id=@company_id and t2.company_id=@company_id) as tbl_1
--       JOIN customers_customer as tbl_2 on tbl_1.cust_id=tbl_2.id) as accode ON dis_tbl.gl_account_id = accode.acc_id) as final
--   WHERE final.acc_code = cc.accode_sal);

-- end$$
-- delimiter ;


-- call update_customer_distributioncode();
