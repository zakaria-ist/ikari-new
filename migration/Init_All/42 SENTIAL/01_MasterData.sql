...
`is_multicurrency`, `profit_loss_group_id`)
select
    ACCTDESC name
...
#IF ACCOUNT CODE IS TRIMMED THEN WE NEED TO RUN THIS
-- update accounts_account acc
-- set code = (select ACCTFMTTD from ikari_db_sage300.glamf where SUBSTRING(ACCTFMTTD , 1, 10) = SUBSTRING(acc.code , 1, 10)) where company_id=@company_id;

update accounts_account acc
set segment_code_id = (select id from companies_costcenters where code = SUBSTRING(acc.code , 6) and company_id=@company_id) where company_id=@company_id;
...
`create_date`, `update_date`, `update_by`, `is_hidden`, `company_id`,
`country_id`, `currency_id`, `customer_id`, `supplier_id`, `account_id`,
`code`)
SELECT
    NAME name
    ,null account_owner
...
SELECT
    ...
    ,(CASE WHEN SCURN = 'DOS' THEN
     ...
     END) currency_id
    ,(select id from accounts_account
     where description = ACCTEXP and company_id=@company_id) expense_account_id
    ,(select id from accounts_account
     where description = LIABILITY and company_id=@company_id) liability_account_id
    ,(select id from accounts_account
     where description = ACCTRECOV and company_id=@company_id) recoverable_account_id
    ,@company_id company_id
...
`ship_via`, `tax_id`, `distribution_id`, `payment_code_id`, `account_set_id`)
SELECT
    vendname name
    ,vendorid code
    ,null address
...
SELECT
    ...
    ,(select id from accounting_paymentcode
        where code= PAYMCODE) payment_code_id
    ,(SELECT id from accounts_accountset
        where code = IDACCTSET
        and company_id = @company_id
     ) account_set_id
...
glafs_loop: LOOP
  ...
  VALUES
    ('1',year,LAST_DAY(CONCAT(year,'-01-01')),
      netprd1, netprd1, netprd1, s_begin1, s_begin1+netprd1,
      fnetprd1, fnetprd1, fnetprd1, f_begin1, f_begin1+fnetprd1,
      now(),now(),NULL,0,acctid,company_currency,currid,company),
    ('2',year,LAST_DAY(CONCAT(year,'-02-01')),
      netprd2, netprd2, netprd2, s_begin2, s_begin2+netprd2,
      fnetprd2, fnetprd2, fnetprd2, f_begin2, f_begin2+fnetprd2,
      now(),now(),NULL,0,acctid,company_currency,currid,company),
    ('3',year,LAST_DAY(CONCAT(year,'-03-01')),
      netprd3, netprd3, netprd3, s_begin3, s_begin3+netprd3,
      fnetprd3, fnetprd3, fnetprd3, f_begin3, f_begin3+fnetprd3,
      now(),now(),NULL,0,acctid,company_currency,currid,company),
    ('4',year,LAST_DAY(CONCAT(year,'-04-01')),
      netprd4, netprd4, netprd4, s_begin4, s_begin4+netprd4,
      fnetprd4, fnetprd4, fnetprd4, f_begin4, f_begin4+fnetprd4,
      now(),now(),NULL,0,acctid,company_currency,currid,company),
    ('5',year,LAST_DAY(CONCAT(year,'-05-01')),
      netprd5, netprd5, netprd5, s_begin5, s_begin5+netprd5,
      fnetprd5, fnetprd5, fnetprd5, f_begin5, f_begin5+fnetprd5,
      now(),now(),NULL,0,acctid,company_currency,currid,company),
    ('6',year,LAST_DAY(CONCAT(year,'-06-01')),
      netprd6, netprd6, netprd6, s_begin6, s_begin6+netprd6,
      fnetprd6, fnetprd6, fnetprd6, f_begin6, f_begin6+fnetprd6,
      now(),now(),NULL,0,acctid,company_currency,currid,company),
    ('7',year,LAST_DAY(CONCAT(year,'-07-01')),
      netprd7, netprd7, netprd7, s_begin7, s_begin7+netprd7,
      fnetprd7, fnetprd7, fnetprd7, f_begin7, f_begin7+fnetprd7,
      now(),now(),NULL,0,acctid,company_currency,currid,company),
    ('8',year,LAST_DAY(CONCAT(year,'-08-01')),
      netprd8, netprd8, netprd8, s_begin8, s_begin8+netprd8,
      fnetprd8, fnetprd8, fnetprd8, f_begin8, f_begin8+fnetprd8,
      now(),now(),NULL,0,acctid,company_currency,currid,company),
    ('9',year,LAST_DAY(CONCAT(year,'-09-01')),
      netprd9, netprd9, netprd9, s_begin9, s_begin9+netprd9,
      fnetprd9, fnetprd9, fnetprd9, f_begin9, f_begin9+fnetprd9,
      now(),now(),NULL,0,acctid,company_currency,currid,company),
    ('10',year,LAST_DAY(CONCAT(year,'-10-01')),
      netprd10, netprd10, netprd10, s_begin10, s_begin10+netprd10,
      fnetprd10, fnetprd10, fnetprd10, f_begin10, f_begin10+fnetprd10,
      now(),now(),NULL,0,acctid,company_currency,currid,company),
    ('11',year,LAST_DAY(CONCAT(year,'-11-01')),
      netprd11, netprd11, netprd11, s_begin11, s_begin11+netprd11,
      fnetprd11, fnetprd11, fnetprd11, f_begin11, f_begin11+fnetprd11,
      now(),now(),NULL,0,acctid,company_currency,currid,company),
    ('12',year,LAST_DAY(CONCAT(year,'-12-01')),
      netprd12, netprd12, netprd12, s_begin12, s_begin12+netprd12,
      fnetprd12, fnetprd12, fnetprd12, f_begin12, f_begin12+fnetprd12,
      now(),now(),NULL,0,acctid,company_currency,currid,company),
    ('ADJ',year,LAST_DAY(CONCAT(year,'-12-01')),
      netprd14, netprd14, netprd14, s_begin14, s_begin14+netprd14,
      fnetprd14, fnetprd14, fnetprd14, f_begin14, f_begin14+fnetprd14,
      now(),now(),NULL,0,acctid,company_currency,currid,company),
    ('CLS',year,LAST_DAY(CONCAT(year,'-12-01')),
      netprd15, netprd15, netprd15, s_begin15, s_begin15+netprd15,
      fnetprd15, fnetprd15, fnetprd15, f_begin15, f_begin15+fnetprd15,
      now(),now(),NULL,0,acctid,company_currency,currid,company);
