...
call change_supplier_code('NDK-Y', 'NHZ-YEN');
call change_supplier_code('NDV-U', 'NDKV-USD');
...
SELECT CNTBTCH batch_no
    ...
    ,(select id from currencies_currency where code='USD') currency_id
...
select t1.* from(select
      ...
      ,(select id from currencies_currency where code='USD') functional_currency_id
...
SELECT DISTINCT
    ...
    0 AS `is_hidden`,
    @company_id AS `company_id`,
    (SELECT `id` FROM `currencies_currency` WHERE `code` = 'USD') AS `currency_id`,
    NULL AS `customer_id`,
    NULL AS `order_id`,
    NULL AS `supplier_id`,
    NULL AS `tax_id`,
...
call change_supplier_code('NHZ-YEN', 'NDK-Y');
call change_supplier_code('NDKV-USD', 'NDV-U');
